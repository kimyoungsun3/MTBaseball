...
--exec spu_FVFarmD 19, 88, 19, -1, -1, -1, -1, -1, -1, -1, 'xxxx2', '', '', '', '', '', '', '', '', ''					-- 닉네임 초기화.
--exec spu_FVFarmD 19, 94, -1, -1, -1, -1, -1, -1, -1, -1, '', '', '', '', '', '', '', '', '', ''						-- 100위
--exec spu_FVFarmD 19, 96, -1, -1, -1, -1, -1, -1, -1, -1, '', '', '', '', '', '', '', '', '', ''						-- 랭킹보기(보기 안보기)
--exec spu_FVFarmD 19, 95, -1, -1, -1, -1, -1, -1, -1, -1, 'xxxx@gmail.com', '', '', '', '', '', '', '', '', ''			-- 클리어
--exec spu_FVFarmD 19, 85, -1, -1, -1, -1, -1, -1, -1, -1, '', '', '', '', '', '', '', '', '', ''						-- 지난랭킹 전체.
--exec spu_FVFarmD 19, 86, -1, -1, -1, -1, -1, -1, -1, -1, '', '', '20150317', '', '', '', '', '', '', ''				-- 			일원.
--exec spu_FVFarmD 19, 97, -1, -1, -1, -1, -1, -1, -1, -1, 'xxxx@gmail.com', '', '', '', '', '', '', '', '', ''			-- 로고기록.
--exec spu_FVFarmD 19, 413, -1, -1, -1, -1, -1, -1, -1, -1, 'xxxx@gmail.com', '', '', '', '', '', '', '', '', ''		-- 초대정보.
--exec spu_FVFarmD 19, 413, -1, -1, -1, -1, -1, -1, -1, -1, '', '', '', '', '', '', '', '', '', ''						-- 초대정보.
...
--exec spu_FVFarmD 19, 2003, -1, -1, -1, -1, -1, -1, -1, -1, 'xxxx@gmail.com', '', '', '', '', '', '', '', '', 'savedata'-- 세이브데이타를 구글로 카피
--exec spu_FVFarmD 19, 2004,  5, -1, -1, -1, -1, -1, -1, -1, '88294959671580064', 'xxxx@gmail.com', '', '', '', '', '', '', '', ''-- 세이브데이타를 구글로 카피
...
Begin
	...
	if(@kind in (@KIND_USER_SEARCH))
		...
		begin
			...
			if(@p2_ = 1)
				...
				begin
					if(@p3_ = 1)
						begin
							-- 쿠폰정보
							set @p4_ = case when @p4_ = -1 then 2 else @p4_ end
			...
			--	end
				...
				begin
					set @comment4 = ''
					select @comment4 = savedata, @gameid = gameid, @market = market from dbo.tFVUserDataBackup where idx = @p3_

					update dbo.tFVUserData
						set
							savedata = @comment4
					where gameid = @gameid and market = @market
		...
		begin
			if(@p2_ in (5, 6))
				...
				begin
					...
					where idx in (select max(idx) from dbo.tFVSystemRouletteMan where roulmarket in (@SKT, @GOOGLE, @IPHONE, @NHN, @LGT) group by roulmarket)
				...
				begin
					...
					select @cnt3 = isnull(max(idx)+1 - min(idx), 0) from dbo.tUserPushAndroid
					select @cnt4 = isnull(max(idx)+1 - min(idx), 0) from dbo.tUserPushiPhone
				...
				begin
					...
					if(@recepushid != '')
						begin
							if(@market = @IPHONE)
								begin
									insert into dbo.tUserPushiPhone(  sendid,  receid,  recepushid, sendkind, msgpush_id, msgtitle, msgmsg, msgaction, scheduleTime)
									values(							@adminid, @gameid, @recepushid,     @p3_,         99,    @ps3_,  @ps4_,     @ps5_, @ps6_)
								end
							else
								begin
									insert into dbo.tUserPushAndroid( sendid,  receid,  recepushid, sendkind, msgpush_id, msgtitle, msgmsg, msgaction, scheduleTime)
									values(							@adminid, @gameid, @recepushid,     @p3_,         99,    @ps3_,  @ps4_,     @ps5_, @ps6_)
								end
						end
					...
					if(@p4_ = @IPHONE)
						begin
							...
							insert into dbo.tUserPushiPhone(recepushid,   sendid, receid, sendkind, msgpush_id, msgtitle, msgmsg, msgaction)
								select distinct                 pushid, @adminid, gameid,     @p3_,         99,    @ps3_,  @ps4_,     @ps5_ from dbo.tUserMaster
							...
							--	메세지 내용기록
							-----------------------------
							insert into dbo.tFVUserPushSendInfo(adminid,  sendkind, market, msgtitle, msgmsg,  cnt, msgurl)
				...
				begin
					...
					-------------------------------------------------
					select count(*) cnt from dbo.tFVItemInfo
					where subcategory in (30, 31, 32, 33)
	...
	-----------------------------------------------------
		...
		begin
			...
			if(@subkind	= 1)
				begin
					select * from dbo.tFVItemInfo where subcategory in (30, 31, 32, 33) and itemcode != 3015
					union all
