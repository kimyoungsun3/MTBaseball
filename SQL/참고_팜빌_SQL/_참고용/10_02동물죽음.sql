/*
exec spu_FVAniDie 'xxxx', '049000s1i0n7t8445287', 1, 12, -1		-- 패스워드 틀림
exec spu_FVAniDie 'xxxx', '049000s1i0n7t8445289', -1, 12, -1		-- 지원하지 않는모드
exec spu_FVAniDie 'xxxx2', '049000s1i0n7t8445289', 1, 18, -1		-- 눌러죽음.
exec spu_FVAniDie 'xxxx2', '049000s1i0n7t8445289', 2, 13, -1		-- 늑대에 죽음.
exec spu_FVAniDie 'xxxx', '049000s1i0n7t8445289', 2, 5, -1		-- 늑대에 죽음.

exec spu_FVAniDie 'xxxx2', '049000s1i0n7t8445289', 1, 1, -1		-- 눌러죽음.
exec spu_FVAniDie 'xxxx2', '049000s1i0n7t8445289', 2, 2, -1		-- 늑대에 죽음.

exec spu_FVAniDie 'xxxx2', '049000s1i0n7t8445289', 1, 18, -1		-- 눌러죽음.
exec spu_FVAniDie 'xxxx2', '049000s1i0n7t8445289', 1, 19, -1		-- 눌러죽음.
exec spu_FVAniDie 'xxxx2', '049000s1i0n7t8445289', 1, 20, -1		-- 눌러죽음.
exec spu_FVAniDie 'xxxx2', '049000s1i0n7t8445289', 1, 21, -1		-- 눌러죽음.
exec spu_FVAniDie 'xxxx2', '049000s1i0n7t8445289', 1, 22, -1		-- 눌러죽음.

select * from dbo.tFVUserItemDel where gameid = 'guest90941'
*/
use Farm
GO

IF OBJECT_ID ( 'dbo.spu_FVAniDie', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FVAniDie;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_FVAniDie
	@gameid_								varchar(60),
...
Begin
	...
	if(isnull(@gameid, '') = '')
		...
		BEGIN
			...
			if(@gameyear >= 2014)
				begin
					insert into dbo.tFVUserItemDieLog(gameid, listidx, invenkind, itemcode, cnt, farmnum, fieldidx, anistep, manger, diseasestate, acc1, acc2, abilkind, abilval, abilkind2, abilval2, abilkind3, abilval3, abilkind4, abilval4, abilkind5, abilval5, randserial, writedate, gethow,   diedate, diemode,  needhelpcnt, petupgrade)
					select                          gameid, listidx, invenkind, itemcode, cnt, farmnum, fieldidx, anistep, manger, diseasestate, acc1, acc2, abilkind, abilval, abilkind2, abilval2, abilkind3, abilval3, abilkind4, abilval4, abilkind5, abilval5, randserial, writedate, gethow, getdate(),  @mode_, @needhelpcnt, petupgrade
					from dbo.tFVUserItem
					where gameid = @gameid_ and listidx = @listidx_ and invenkind in (@USERITEM_INVENKIND_ANI)
				end
			...
			select listidx from dbo.tFVUserItem
				...
				Begin
					...
					if(@loop >= @GAME_INVEN_HOSPITAL_BASE)
						begin
							...
							exec spu_FVDeleteUserItemBackup 0, @gameid_, @listidx
						end
