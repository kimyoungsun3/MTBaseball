...
IF OBJECT_ID ( 'dbo.spu_FVGameTradeTest', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FVGameTradeTest;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_FVGameTradeTest
	@gameid_								varchar(60),
...
as
	...
	declare @TRADEINFO_SALEPLUSDANGA_MAX		int					set @TRADEINFO_SALEPLUSDANGA_MAX			= 300	--100 현재맥스(49)0번상인 (단가:6118코인 + 추가:1223코인) x 11배럴(32신선도) = 판매금:80751
...
Begin
	...
	-- 거래필터 추가.(기존것 + 올드)
		...
		BEGIN
			...
			--       코인 = 원래코인  + 게임중 획득코인(검사완) + 상장코인(6개월마다:검사완) + 판매수익(검사완)
			set @gamecost = @gamecost + @playcoin2              + @prizecoin2				 + @salecoin2
			...
			if(@famelvchange = 1 and @famelv in (3, 6, 9, 12))
				begin
					if(@famelv = 3)
						begin
	...
	if(@nResult_ = @RESULT_SUCCESS)
		BEGIN
			...
			---- 없으면 > 빈리스트를 전송해주기
			----------------------------------------------------------------
			--if(@saleitemcode2 != -1 or @etreward != -1)
			--	begin
