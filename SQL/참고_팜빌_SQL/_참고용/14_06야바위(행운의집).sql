...
/*
-- 야바위 리스트 갱신.
update dbo.tFVUserMaster set cashcost = 10000, gamecost = 100000, yabauidx = 1, yabaustep = 0, yabaunum = -1, randserial = -1 where gameid = 'xxxx2'
exec spu_FVRoulYabau 'xxxx2', '049000s1i0n7t8445289', 1,     -1, -1	-- 리스트 갱신
exec spu_FVRoulYabau 'farm939089965', '9826449f4y8p8t884474', 1,     -1, -1

-- 야바위 뽑기.
update dbo.tFVUserMaster set cashcost = 0, gamecost = 0, yabauidx = 1, yabaustep = 0, yabaunum = -1, randserial = -1 where gameid = 'xxxx2'
update dbo.tFVUserMaster set cashcost = 10000, gamecost = 100000, yabauidx = 1, yabaustep = 0, yabaunum = -1, randserial = -1 where gameid = 'xxxx2'
exec spu_FVRoulYabau 'xxxx2', '049000s1i0n7t8445289', 2, 11111, -1	-- 일반야바위
exec spu_FVRoulYabau 'xxxx2', '049000s1i0n7t8445289', 2, 11112, -1	-- 일반야바위
exec spu_FVRoulYabau 'xxxx2', '049000s1i0n7t8445289', 3, 22222, -1	-- 프리미엄야바위
exec spu_FVRoulYabau 'xxxx2', '049000s1i0n7t8445289', 3, 22223, -1	-- 프리미엄야바위

-- 야바위 받아가기 > 리스트 갱신됨.
delete from dbo.tFVGiftList where gameid in ('xxxx2')
update dbo.tFVUserMaster set yabauidx = 1, yabaucount = 0, yabaustep = 1 where gameid = 'xxxx2'
exec spu_FVRoulYabau 'xxxx2', '049000s1i0n7t8445289', 4,     40, -1	-- 받아가기 갱신
*/
use Farm
GO

IF OBJECT_ID ( 'dbo.spu_FVRoulYabau', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FVRoulYabau;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_FVRoulYabau
	@gameid_				varchar(60),
...
Begin
	...
	from dbo.tFVSystemYabau where idx = @yabauidx
		...
		BEGIN
			if (@gamecost < @yabauchange)
				...
				BEGIN
					...
					-- 구매로그 기록.
					------------------------
					exec spu_FVUserItemBuyLog @gameid_, @ITEMCODE_YABAU_CHANGE, @yabauchange, 0

					------------------------
					-- 야바위로그(변경).
					------------------------
					insert into dbo.tFVYabauLogPerson(gameid,   itemcode, kind,   framelv,  yabauchange,  yabaucount, remaingamecost, remaincashcost)
		...
		BEGIN
			if (@yabaustep < 0 or @yabaustep > 6)
				...
				BEGIN
					...
					-- 선물함에 넣어주기.
					------------------------------------------------------------------
					if(@pack11 != -1)exec spu_FVSubGiftSend @GIFTLIST_GIFT_KIND_GIFT,  @pack11, 'SysBabau', @gameid_, ''
					if(@pack21 != -1)exec spu_FVSubGiftSend @GIFTLIST_GIFT_KIND_GIFT,  @pack21, 'SysBabau', @gameid_, ''
					if(@pack31 != -1)exec spu_FVSubGiftSend @GIFTLIST_GIFT_KIND_GIFT,  @pack31, 'SysBabau', @gameid_, ''
					if(@pack41 != -1)exec spu_FVSubGiftSend @GIFTLIST_GIFT_KIND_GIFT,  @pack41, 'SysBabau', @gameid_, ''
					if(@pack51 != -1)exec spu_FVSubGiftSend @GIFTLIST_GIFT_KIND_GIFT,  @pack51, 'SysBabau', @gameid_, ''
					if(@pack61 != -1)exec spu_FVSubGiftSend @GIFTLIST_GIFT_KIND_GIFT,  @pack61, 'SysBabau', @gameid_, ''

					--------------------------------------
					-- 기록하기.
					--------------------------------------
					-- 월기록.
					if(not exists(select top 1 * from dbo.tFVUserYabauMonth where dateid6 = @dateid6 and itemcode = @itemcode))
						begin
							insert into dbo.tFVUserYabauMonth(dateid6, itemcode) values(@dateid6, @itemcode)
						end
					update dbo.tFVUserYabauMonth
						set
					...
					-- 일기록.
					if(not exists(select top 1 * from dbo.tFVUserYabauTotalSub where dateid8 = @dateid8 and itemcode = @itemcode))
						begin
							insert into dbo.tFVUserYabauTotalSub(dateid8, itemcode) values(@dateid8, @itemcode)
						end
					update dbo.tFVUserYabauTotalSub
						set
							step1 = step1 + case when (@yabaustep = 1) then 1 else 0 end,
							step2 = step2 + case when (@yabaustep = 2) then 1 else 0 end,
							step3 = step3 + case when (@yabaustep = 3) then 1 else 0 end,
					...
					-- 야바위로그(받기).
					------------------------
					insert into dbo.tFVYabauLogPerson(gameid,  itemcode,   kind,  framelv,  pack11,  pack21,  pack31,  pack41,  pack51,  pack61,  yabaustep,  yabaucount, remaingamecost, remaincashcost)
				...
				BEGIN
					...
					if(@mode_ = @MODE_YABAU_NORMAL)
						begin
							...
							-- 구매로그 기록.
							------------------------
							exec spu_FVUserItemBuyLog @gameid_, @itemcode, @needgamecost, 0
						end
						...
						begin
							...
							-- 구매로그 기록.
							------------------------
							exec spu_FVUserItemBuyLog @gameid_, @itemcode, 0, @needgcashcost
						end
					...
					insert into dbo.tFVYabauLogPerson(gameid,   itemcode,  kind,  framelv,  pack11,  pack21,  pack31,  pack41,  pack51,  pack61,       result,       cashcost,      gamecost,  yabaustep,  yabaucount, remaingamecost, remaincashcost)
