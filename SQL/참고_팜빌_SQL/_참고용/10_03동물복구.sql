/*
-- delete from dbo.tFVUserItem where gameid = 'xxxx5' and invenkind = 1
exec spu_FVAniRestore 'xxxx5', '049000s1i0n7t8445289', -1		-- 복구하기.
*/
use Farm
GO

IF OBJECT_ID ( 'dbo.spu_FVAniRestore', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FVAniRestore;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_FVAniRestore
	@gameid_								varchar(60),
...
Begin
	...
	if(@gameid = '')
		...
		BEGIN
			...
			select @listidx = isnull(MAX(listidx), 0) + 1 from dbo.tFVUserItem where gameid = @gameid_
			set @loop = case
			...
			while(@loop > 0)
				begin
					...
					values(					 @gameid_, @listidx, @ITEM_RESTORE_ANIMAL,   1,       0, @fieldidx, @USERITEM_INVENKIND_ANI,          0, @DEFINE_HOW_GET_FIRST)
