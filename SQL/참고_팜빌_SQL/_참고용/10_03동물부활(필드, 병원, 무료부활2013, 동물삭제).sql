...
exec spu_FVAniDie 'xxxx2', '049000s1i0n7t8445289', 2, 3, -1
exec spu_FVAniDie 'xxxx2', '049000s1i0n7t8445289', 3, 4, -1
exec spu_FVAniDie 'xxxx2', '049000s1i0n7t8445289', 4, 6, -1
...
exec spu_FVAniRevival 'xxxx2', '049000s1i0n7t8445289', 2, 3, 3, -1		-- 2013년 무료부활

-- 부활석x1 or 캐쉬+1
exec spu_FVAniRevival 'xxxx2', '049000s1i0n7t8445289', 1, 1, 1, -1		-- 필드부활 > 필드로 넣기.
exec spu_FVAniRevival 'xxxx2', '049000s1i0n7t8445289', 1, 3, 2, -1		--
exec spu_FVAniRevival 'xxxx2', '049000s1i0n7t8445289', 1, 4, 4, -1		-- (클라죽음 - 서버살음) 싱크 안맞음 > 클라기준으로 살려줌.
exec spu_FVAniRevival 'xxxx2', '049000s1i0n7t8445289', 1, 3, -1, -1		-- 필드부활 > 필드로 넣기.

-- 부활석x2 or (캐쉬+1)*2
exec spu_FVAniRevival 'xxxx2', '049000s1i0n7t8445289', 2, 19, -1, -1		-- 병원부활 > 인벤으로 넣기.
exec spu_FVAniRevival 'xxxx2', '049000s1i0n7t8445289', 2, 6, -1, -1		--

-- 삭제하기
exec spu_FVAniRevival 'xxxx', '049000s1i0n7t8445289', 3, 5, -1, -1		-- 병원부활 > 삭제하기.
*/
use Farm
GO

IF OBJECT_ID ( 'dbo.spu_FVAniRevival', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FVAniRevival;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_FVAniRevival
	@gameid_								varchar(60),
...
Begin
	...
	--	END
		...
		BEGIN
			...
			--select 'DEBUG ' + @comment
		END
	else if(@anilistidx2 != @listidx_ and @fieldidx_ != @USERITEM_FIELDIDX_INVEN and exists(select top 1 * from dbo.tFVUserItem where gameid = @gameid_ and fieldidx = @fieldidx_))
		BEGIN
			set @nResult_ 	= @RESULT_ERROR_ANIMAL_IS_ALIVE
		...
		BEGIN
			...
			--select 'DEBUG ' + @comment
			if(exists(select top 1 * from dbo.tFVUserItem where gameid = @gameid_ and listidx = @listidx_))
				begin
					exec spu_FVDeleteUserItemBackup 0, @gameid_, @listidx_
				end
		END
	else
		BEGIN
			...
			if(@revcntneed = 0)
				begin
					...
					-- 유저 정보 수집.
					-----------------------------------
					exec spu_FVDayLogInfoStatic @market, 25, @revcntneed				-- 무료부활.
				...
				begin
					...
					-- 유저 정보 수집.
					-----------------------------------
					exec spu_FVDayLogInfoStatic @market, 23, @revcntneed				-- 일 부활수(템)
				...
				begin
					...
					-- 유저 정보 수집.
					-----------------------------------
					exec spu_FVDayLogInfoStatic @market, 24, @revcashcostneed			-- 일 부활수(캐쉬)
					...
					-- 구매기록마킹
					-----------------------------------
					exec spu_FVUserItemBuyLog @gameid_, @REVIVAL_ITEMCODE, 0, @revcashcostneed
				end
			else
				begin
			...
			if(@nResult_ = @RESULT_SUCCESS and @alivecheck = 1)
				begin
					insert into dbo.tFVUserItemAliveLog(gameid, listidx, invenkind, itemcode, cnt, farmnum, fieldidx, anistep, manger, diseasestate, acc1, acc2, abilkind, abilval, abilkind2, abilval2, abilkind3, abilval3, abilkind4, abilval4, abilkind5, abilval5, randserial, writedate, gethow,   diedate, diemode,  needhelpcnt, petupgrade, alivedate,  alivecash,  alivedoll)
					select                            gameid, listidx, invenkind, itemcode, cnt, farmnum, fieldidx, anistep, manger, diseasestate, acc1, acc2, abilkind, abilval, abilkind2, abilval2, abilkind3, abilval3, abilkind4, abilval4, abilkind5, abilval5, randserial, writedate, gethow,   diedate, diemode,  needhelpcnt, petupgrade, getdate(), @alivecash, @alivedoll
					from dbo.tFVUserItem
					where gameid = @gameid_ and listidx = @listidx_
				end
		END
	...
	if(@nResult_ = @RESULT_SUCCESS)
		...
		begin
			select @nResult_ rtn, @comment comment, @gamecost gamecost, @cashcost cashcost
		end
