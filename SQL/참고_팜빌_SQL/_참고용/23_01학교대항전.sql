/*
-- 1. 검색.
exec spu_FVSchoolInfo 'xxxx2', '049000s1i0n7t8445289', 1, 1, '운현', -1		-- 검색모드(초등학교, 용봉)

-- 2-1. 최초가입 가입.
-- select gameid, password, schoolidx from dbo.tFVUserMaster where idx >= 1092 and idx <= 1092 + 20
-- delete from dbo.tFVSchoolUser where gameid = 'xxxx2'
-- update dbo.tFVUserMaster set schoolidx = -1, schoolresult = -1 where gameid = 'xxxx2'
exec spu_FVSchoolInfo 'xxxx2', '049000s1i0n7t8445289', 2, 10709, '', -1			-- 가입모드

-- 2-2. 점수획득
select * from dbo.tFVSchoolMaster where schoolidx in (23, 24)
select * from dbo.tFVSchoolUser where schoolidx in (23, 24) order by schoolidx desc

-- 2-3. 다른친구 가입
-- update dbo.tFVUserMaster set schoolidx = -1, schoolresult = -1 where gameid like 'xxxx%'
-- delete from dbo.tFVSchoolUser where gameid like 'xxxx%'
exec spu_FVSchoolInfo 'xxxx', '049000s1i0n7t8445289', 2, 23, '', -1			-- 가입모드
exec spu_FVSchoolInfo 'xxxx2', '049000s1i0n7t8445289', 2, 23, '', -1			-- 가입모드
exec spu_FVSchoolInfo 'xxxx3', '049000s1i0n7t8445289', 2, 23, '', -1			-- 가입모드
exec spu_FVSchoolInfo 'xxxx4', '049000s1i0n7t8445289', 2, 23, '', -1			-- 가입모드
exec spu_FVSchoolInfo 'xxxx5', '049000s1i0n7t8445289', 2, 23, '', -1			-- 가입모드
exec spu_FVSchoolInfo 'xxxx6', '049000s1i0n7t8445289', 2, 23, '', -1			-- 가입모드
exec spu_FVSchoolInfo 'xxxx7', '049000s1i0n7t8445289', 2, 23, '', -1			-- 가입모드
exec spu_FVSchoolInfo 'xxxx8', '049000s1i0n7t8445289', 2, 23, '', -1			-- 가입모드
exec spu_FVSchoolInfo 'xxxx9', '049000s1i0n7t8445289', 2, 23, '', -1			-- 가입모드
exec spu_FVSchoolInfo 'xxxx10', '049000s1i0n7t8445289', 2, 23, '', -1			-- 가입모드
exec spu_FVSchoolInfo 'xxxx11', '049000s1i0n7t8445289', 2, 23, '', -1			-- 가입모드

-- 3. 학교 Top 10위 + 내순위
exec spu_FVSchoolInfo 'xxxx2', '049000s1i0n7t8445289', 3, -1, '', -1

-- 4. 소속 Top 30위 + 내순위
exec spu_FVSchoolInfo 'xxxx2', '049000s1i0n7t8445289', 4, -1, '', -1
exec spu_FVSchoolInfo 'xxxx2', '049000s1i0n7t8445289', 4,  1, '', -1
...
select @schoolidx = schoolidx, @schoolresult = schoolresult from dbo.tFVUserMaster where gameid = @gameid
select @schoolidx schoolidx, @schoolresult schoolresult
select * from dbo.tFVSchoolBank where schoolidx = @schoolidx
...
IF OBJECT_ID ( 'dbo.spu_FVSchoolInfo', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FVSchoolInfo;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_FVSchoolInfo
...
Begin
	...
	if(@gameid = '')
		...
		BEGIN
			...
			select top 10 * from dbo.tFVSchoolBank
		...
		BEGIN
			...
			-- 현재 학교순위       (           학교순위 + MY             )
			--------------------------------------------------------------
			exec spu_FVSchoolRank  4, -1, @gameid_
		END
		...
		BEGIN
			...
			if(@paramint_ != -1)
				begin
					exec spu_FVSchoolRank  7, @paramint_, ''
				end
			else
				begin
					exec spu_FVSchoolRank  5, -1, @gameid_
				end
		END
		...
		BEGIN
			...
			select @cnt  = cnt from dbo.tFVSchoolMaster where schoolidx = @schoolidxnew
			...
			else if(not exists(select top 1 * from dbo.tFVSchoolBank where schoolidx = @schoolidxnew))
				begin
					set @nResult_ 	= @RESULT_ERROR_CANNT_FIND_SCHOOL
				...
				begin
					...
					if(@schoolidxcur != -1)
						begin
							...
							--select 'DEBUG 1-1. 기존가입 탈퇴 > 인력감소, 포인트감소', cnt, totalpoint from dbo.tFVSchoolMaster where schoolidx = @schoolidxcur
						end
					else
						begin
							set @point = @point
					...
					-- 2-1. 마스터 가입
					-----------------------
					if(not exists(select top 1 * from dbo.tFVSchoolMaster where schoolidx = @schoolidxnew))
						begin
							-- 학교가 새롭게 들어오면 생성해준다.
					...
					--select 'DEBUG 2-1. 마스터 인원추가', @schoolidxnew schoolidxnew, @gameid_ gameid_, @point point, cnt, totalpoint from dbo.tFVSchoolMaster where schoolidx = @schoolidxnew

					-----------------------
					-- 2-2. 유저 가입하기.
					-----------------------
					if(not exists(select top 1 * from dbo.tFVSchoolUser where gameid = @gameid_))
						begin
							--select 'DEBUG 3-1. 학교유저서브 insert'
							insert into dbo.tFVSchoolUser(schoolidx,     gameid,   point)
			...
			if( @nResult_ = @RESULT_SUCCESS)
				begin
					-- 학교이름.
					set @schoolname = ''
					select @schoolname = isnull(schoolname, '') from dbo.tFVSchoolBank where schoolidx = @schoolidxnew
