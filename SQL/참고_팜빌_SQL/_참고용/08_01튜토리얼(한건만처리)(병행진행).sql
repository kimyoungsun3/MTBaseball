...
IF OBJECT_ID ( 'dbo.spu_FVTutorial', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FVTutorial;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_FVTutorial
	@gameid_								varchar(60),
	...
	if(isnull(@gameid, '') = '')
		...
		BEGIN
			...
			if(@tutorial = @MODE_TUTORIAL_OK)
				...
				begin
					-- 4수정 + 150코인 + 2 캐시 일꾼 + 2캐시 치료제
					exec spu_FVSubGiftSend 2, 5008, 'SysTut', @gameid_, ''
					exec spu_FVSubGiftSend 2, 5102, 'SysTut', @gameid_, ''
					exec spu_FVSubGiftSend 2,  803, 'SysTut', @gameid_, ''
					exec spu_FVSubGiftSend 2, 1003, 'SysTut', @gameid_, ''
					set @tutorial	= @MODE_TUTORIAL_OK
