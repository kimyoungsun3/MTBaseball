/*
exec spu_FVSchoolRank  1, -1, ''			-- 현재 학교순위       (           학교순위                  )					--										50_01웹검색
exec spu_FVSchoolRank  4, -1, 'xxxx2'		-- 현재 학교순위       (           학교순위 + MY(가입자)     )					-- 13_02게임거래	23_01학교대항전
exec spu_FVSchoolRank  4, -1, 'guest90909'-- 현재 학교순위       (           학교순위 + MY(미가입자)   )					-- 13_02게임거래	23_01학교대항전
exec spu_FVSchoolRank  6, -1, 'xxxx2'     -- 현재 학교순위       (           학교순위 + MY(깡통)		 )					-- 13_02게임거래
exec spu_FVSchoolRank  2, 6607, ''		-- 현재 학교내 유저순위(           학교번호 > 학교인원들 순위) > Web에서 사용	--										50_01웹검색
exec spu_FVSchoolRank  3, -1, 'xxxx2'		-- 현재 학교내 유저순위(유저이름 > 학교번호 > 학교인원들 순위)					--										50_01웹검색
exec spu_FVSchoolRank  5, -1, 'xxxx2'		-- 현재 학교내 유저순위(유저이름 > 학교번호 > 학교인원들 순위 + MY)				--					23_01학교대항전
exec spu_FVSchoolRank  7, 6607, ''		-- 현재 학교내 유저순위(           학교번호 > 학교인원들 순위) > 게임에서 사용	-- 13_02게임거래	23_01학교대항전
exec spu_FVSchoolRank  7, -1, ''			-- 현재 학교내 유저순위(           학교번호 > 학교인원들 순위) > 더미			-- 13_02게임거래	23_01학교대항전

exec spu_FVSchoolRank 11, -1, 'xxxx2'		-- 지난 학교랭킹(학교 + 내소속) > LOGIN											-- 04_01로그인
exec spu_FVSchoolRank 11, -1, 'xxxx2'	--																				-- 04_01로그인
-- select * from dbo.tFVEpiReward where gameid = 'guest198' order by idx asc
-- exec spu_FVLogin 'guest198', '3023677g3i8n7l492451', 1, 101, '', '', -1, -1			-- 정상유저


exec spu_FVSchoolRank 20, -1, ''			-- 지난 날짜리스트.																--										50_01웹검색
exec spu_FVSchoolRank 22, -1, '20140523'	-- 지난 지정된 날짜 학교순위 (20131227일 학교 랭킹)								--										50_01웹검색
exec spu_FVSchoolRank 23,  1, '20140523'	-- 지난 지정된 날짜 학교내의 > 유저 랭킹										--										50_01웹검색
*/
use Farm
GO

IF OBJECT_ID ( 'dbo.spu_FVSchoolRank', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FVSchoolRank;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_FVSchoolRank
	@mode_									int,
	@paramint_								int,
...
Begin
	...
	if(@mode_ = @SCHOOLRANK_CURRENT_SCHOOLRANK)
		BEGIN
			...
			select top 500 rank() over(order by totalpoint desc) schoolrank, schoolidx, cnt, totalpoint/100 from dbo.tFVSchoolMaster
			--where totalpoint > 0
			...
			JOIN
				(select * from dbo.tFVSchoolBank where schoolidx in (select schoolidx from @tTempTableCurrentMaster)) b
			ON
		...
		BEGIN
			...
			set @totalpoint	= 0
			select @schoolidx = schoolidx, @cnt = cnt, @totalpoint = totalpoint from dbo.tFVSchoolMaster
			where schoolidx = (select schoolidx from dbo.tFVUserMaster where gameid = @gameid)
			...
			select top 10 rank() over(order by totalpoint desc) schoolrank,  schoolidx, cnt, totalpoint/100 from dbo.tFVSchoolMaster
			union
			select count(schoolidx) + 1 as schoolrank, @schoolidx schoolidx, @cnt cnt, @totalpoint/100 totalpoint from dbo.tFVSchoolMaster where totalpoint > @totalpoint

			select m.*, b.schoolname, schoolarea, schoolkind from
				@tTempTableCurrentMasterMy m
			JOIN
				(select * from dbo.tFVSchoolBank where schoolidx in (select schoolidx from @tTempTableCurrentMasterMy)) b
			ON
				m.schoolidx = b.schoolidx
			--order by schoolrank asc
		END
		...
		BEGIN
			...
			select top 500 rank() over(order by point desc) userrank, @schoolname schoolname, * from dbo.tFVSchoolUser
		...
		BEGIN
			...
			--select 'DEBUG 내가 속한 그룹인원들 실시간 순위'
			---------------------------------------------
			select @schoolidx = schoolidx from dbo.tFVUserMaster where gameid = @paramstr_
			select @schoolname = schoolname from dbo.tFVSchoolBank where schoolidx = @schoolidx
		...
		BEGIN
			...
			select @schoolname = schoolname from dbo.tFVSchoolBank where schoolidx = @schoolidx
			select @point = point from dbo.tFVSchoolUser where gameid = @gameid and schoolidx = @schoolidx and schoolidx != -1
			...
			select top 10 rank() over(order by point desc) userrank, gameid, point, @schoolname schoolname, schoolidx, @cnt cnt from dbo.tFVSchoolUser where schoolidx = @schoolidx and schoolidx != -1
			union
			select count(gameid) + 1 as userrank, @gameid gameid, @point point, @schoolname schoolname, @schoolidx schoolidx, @cnt cnt from dbo.tFVSchoolUser where schoolidx = @schoolidx and point > @point and schoolidx != -1
			--order by point desc
			...
			select r.*, isnull(itemcode, 1) itemcode, isnull(acc1, -1) acc1, isnull(acc2, -1) acc2, kakaotalkid, kakaouserid, kakaonickname, kakaoprofile, kakaomsgblocked, 1 kakaofriendkind from
				(select gameid, anireplistidx, kakaotalkid, kakaouserid, kakaonickname, kakaoprofile, kakaomsgblocked from dbo.tFVUserMaster where gameid in (select gameid from @tTempUserRank)) as m
			LEFT JOIN
				(select gameid, listidx, itemcode, acc1, acc2 from dbo.tFVUserItem where gameid in (select gameid from @tTempUserRank)) as i
			ON
		...
		BEGIN
			...
			select top 10 rank() over(order by point desc) userrank, gameid, point, @schoolname schoolname, schoolidx, @cnt cnt from dbo.tFVSchoolUser
			where schoolidx = @schoolidx and schoolidx != -1
			--order by point desc

			select r.*, isnull(itemcode, 1) itemcode, isnull(acc1, -1) acc1, isnull(acc2, -1) acc2, kakaotalkid, kakaouserid, kakaonickname, kakaoprofile, kakaomsgblocked, 1 kakaofriendkind from
				(select gameid, anireplistidx, kakaotalkid, kakaouserid, kakaonickname, kakaoprofile, kakaomsgblocked from dbo.tFVUserMaster where gameid in (select gameid from @tTempUserRank2)) as m
			LEFT JOIN
				(select gameid, listidx, itemcode, acc1, acc2 from dbo.tFVUserItem where gameid in (select gameid from @tTempUserRank2)) as i
			ON
				m.gameid = i.gameid and m.anireplistidx = i.listidx
			JOIN
		...
		BEGIN
			...
			select top 1 @dateid = dateid from dbo.tFVSchoolBackMaster order by dateid desc

			-- 지난주 학교랭킹
			insert into @tTempTableLastWeekMaster
			select top 3 dateid, schoolrank, schoolidx, cnt, totalpoint/100
			from dbo.tFVSchoolBackMaster
			where dateid = @dateid
			order by schoolrank asc
			...
			JOIN
				(select schoolidx, schoolname from dbo.tFVSchoolBank where schoolidx in (select schoolidx from @tTempTableLastWeekMaster)) b
			ON
			...
			select @schoolname = schoolname from dbo.tFVSchoolBank where schoolidx = @schoolidx
			select @cnt = cnt from dbo.tFVSchoolBackMaster where dateid = @dateid and schoolidx = @schoolidx
		...
		BEGIN
			...
			---------------------------------------------
			select top 500 * from dbo.tFVSchoolSchedule
		...
		BEGIN
			...
			select top 500 dateid, schoolrank, schoolidx, cnt, totalpoint/100 from dbo.tFVSchoolBackMaster
			where dateid = @dateid
			order by schoolrank asc
			...
			JOIN
				(select schoolidx, schoolname from dbo.tFVSchoolBank where schoolidx in (select schoolidx from @tTempTableLastSchoolList)) b
			ON
				l.schoolidx = b.schoolidx
			order by schoolrank asc
		END
		...
		BEGIN
			...
			select top 500 backdateid, backschoolrank, backuserrank, backschoolidx, gameid, joindate, backpoint, backitemcode, backacc1, backacc2, backitemcode1, backitemcode2, backitemcode3 from dbo.tFVSchoolUser
			...
			JOIN
				(select schoolidx, schoolname from dbo.tFVSchoolBank where schoolidx in (select schoolidx from @tTempTableLastUserList)) b
			ON
				l.schoolidx = b.schoolidx
			--order by schoolrank asc
		END
