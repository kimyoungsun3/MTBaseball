...
update dbo.tFVUserMaster set gamecost = 100000, gameyear = 2826 where gameid = 'xxxx2'
update dbo.tFVUserFarm set buystate = -1 where gameid = 'xxxx2' and itemcode = 6930
exec spu_FVUserFarm 'xxxx2', '049000s1i0n7t8445289',  1, 6930, -1		-- 구매

update dbo.tFVUserFarm set incomedate = getdate() - 0.1, buystate = 1 where gameid = 'xxxx2' and itemcode = 6900
exec spu_FVUserFarm 'xxxx2', '049000s1i0n7t8445289',  2, 6900, -1		-- 수입

update dbo.tFVUserFarm set incomedate = getdate() - 1 where gameid = 'xxxx2'
update dbo.tFVUserMaster set gamecost = 0 where gameid = 'xxxx2'
exec spu_FVUserFarm 'xxxx2', '049000s1i0n7t8445289',  22, -1, -1		-- All수입

update dbo.tFVUserFarm set incomedate = getdate() - 0.1, buystate = 1 where gameid = 'xxxx2' and itemcode = 6900
exec spu_FVUserFarm 'xxxx2', '049000s1i0n7t8445289',  3, 6900, -1		-- 판매
*/
use Farm
GO

IF OBJECT_ID ( 'dbo.spu_FVUserFarm', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FVUserFarm;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_FVUserFarm
...
Begin
	...
	--select 'DEBUG 농장 일반정보', @gamecostorg gamecostorg, @hourcoin hourcoin, @maxcoin maxcoin, @raiseyear raiseyear, @raisepercent raisepercent
		...
		BEGIN
			...
			if(@gamecost < @gamecostcur)
				...
				BEGIN
					...
					-- 목장 구매 상태로 변경
					---------------------------------------------
					--select 'DEBUG 전', @@ROWCOUNT, * from dbo.tFVUserFarm where gameid = @gameid_ and itemcode = @itemcode_
					update dbo.tFVUserFarm
						set
							...
							buycount	= buycount + 1
					where gameid = @gameid_ and itemcode = @itemcode_
					--select 'DEBUG 후', @@ROWCOUNT, * from dbo.tFVUserFarm where gameid = @gameid_ and itemcode = @itemcode_
					...
					if(@buycount = 0)
						begin
							exec spu_FVUserItemBuyLog @gameid_, @itemcode_, @gamecostcur, 0
						end
					...
					if(@rewarditemcode != -1)
						begin
							exec spu_FVSubGiftSend @GIFTLIST_GIFT_KIND_GIFT,  @rewarditemcode, 'FarmBuy', @gameid_, ''
						end
	...
	if(@nResult_ = @RESULT_SUCCESS)
		begin
			...
			-- 농장리스트 전송
			--------------------------------------------------------------
			exec spu_FVUserFarmListNew @gameid_, 1, @market, @version
