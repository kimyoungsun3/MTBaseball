...
IF OBJECT_ID ( 'dbo.spu_FVSamplePro', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FVSamplePro;
GO

create procedure dbo.spu_FVSamplePro
...
Begin
	...
	-- 2개 이상적용됨(테이블로 들어와서)
	-----------------------------------------------
	--declare @gameid_ varchar(60) set @gameid_ = 'SangSang'
	DECLARE @tItemExpire TABLE(
		idx bigint,
		gameid varchar(20),
		itemcode int
	);
	--select * from dbo.tFVUserItem where gameid = @gameid_
	...
	declare curQuestUser Cursor for
		select questkind, questsubkind, questclear from dbo.tFVQuestInfo
			...
			Begin
				if(@questclear = @QUEST_CLEAR_START)
					begin
						...
						-- exec spu_FVQuestCheck 1, 'superman3', 1000, 8, -1, -1
						exec spu_FVQuestCheck @QUEST_MODE_CLEAR, @gameid_, @questkind, @questsubkind, -1, -1
					end
				Fetch next from curQuestUser into @questkind, @questsubkind, @questclear
			end
		close curQuestUser
