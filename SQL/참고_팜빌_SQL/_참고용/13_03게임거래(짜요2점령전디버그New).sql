		...
		BEGIN
			...
			if(@famelvchange = 1 and @famelv in (3, 6, 9, 12, 50, 55, 60, 65, 70))
				begin
					if(@famelv = 3)
						begin
							exec spu_FVSubGiftSend @GIFTLIST_GIFT_KIND_GIFT,  @EVENT01_ITEM1, 'LvUpEvent', @gameid_, '레벨업이벤트3'
						end
					else if(@famelv = 6)
						begin
							exec spu_FVSubGiftSend @GIFTLIST_GIFT_KIND_GIFT,  @EVENT01_ITEM2, 'LvUpEvent', @gameid_, '레벨업이벤트6'
						end
					else if(@famelv = 9)
						begin
							exec spu_FVSubGiftSend @GIFTLIST_GIFT_KIND_GIFT,  @EVENT01_ITEM3, 'LvUpEvent', @gameid_, '레벨업이벤트9'
						end
					else if(@famelv = 12)
						begin
							exec spu_FVSubGiftSend @GIFTLIST_GIFT_KIND_GIFT,  @EVENT01_ITEM4, 'LvUpEvent', @gameid_, '레벨업이벤트12'
						end

					if(@curdate >= '2014-09-19')
						begin
							if(@famelv in (50))
								begin
									-- 50레벨 - 수정5
