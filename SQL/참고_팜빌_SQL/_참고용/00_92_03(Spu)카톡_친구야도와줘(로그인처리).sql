-----------------------------------------------------------------------
-- select top 1 * from dbo.tFVKakaoHelpWait where gameid = 'xxxx'
-- delete from dbo.tFVGiftList where gameid in ('xxxx2', 'xxxx', 'xxxx3')
...
IF OBJECT_ID ( 'dbo.spu_FVsubKakaoHelpWait', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FVsubKakaoHelpWait;
GO

create procedure dbo.spu_FVsubKakaoHelpWait
	@gameid_				varchar(60)
...
Begin
	...
	-- 닉네임을 얻어오기.
	select @kakaonickname = kakaonickname FROM dbo.tFVUserMaster where gameid = @gameid_

	-- 1. 커서 생성
	declare curKakaoHelpWait Cursor for
	select friendid, listidx FROM dbo.tFVKakaoHelpWait where gameid = @gameid_
	...
	while @@Fetch_status = 0
		Begin
			...
			select @needhelpcnt = needhelpcnt, @invenkind = invenkind, @itemcode = itemcode from dbo.tFVUserItem where gameid = @friendid and listidx = @listidx
			...
			if(@needhelpcnt <= 0 or @needhelpcnt = 99999 or @invenkind != @USERITEM_INVENKIND_ANI)
				...
				begin
					...
					Exec Spu_Subgiftsend 1, -1, @helpgameid, @friendid, @helpcomment
					...
					----	3. 악세사리를 넣어준다.
					------------------------------------------------
					--delete from dbo.tFVUserItem where gameid = @friendid and listidx = @listidx
                    --
					--set @helpgameid = '친구도움(' + @kakaonickname + ')'
					--Exec spu_FVSubGiftSend 2, @itemcode, @helpgameid, @friendid, ''				-- 젖소
	...
	if(@loop >= 1)
		begin
			...
			delete from dbo.tFVKakaoHelpWait where gameid = @gameid_
		end

	if(@ishelp = 1)
		begin
			-- 도움을 준 친구에게 보상해주기.
			exec spu_FVSubGiftSend 2, @HEART_REWARD, 'SysHelp2', @gameid_, ''
		end
