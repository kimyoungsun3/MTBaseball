/*
update dbo.tFVUserMaster set boardwrite = getdate() - 0.01 where gameid = 'xxxx2'
exec spu_FVUserBoard 'xxxx2', '049000s1i0n7t8445289', 1, 1, -1, '친추게시판광고', -1		-- 게시판글쓰기.
exec spu_FVUserBoard 'xxxx2', '049000s1i0n7t8445289', 1, 2, -1, '일반게시판광고', -1		-- < 사용안함.
exec spu_FVUserBoard 'xxxx2', '049000s1i0n7t8445289', 1, 3, -1, '대항게시판광고', -1

-- delete from dbo.tFVUserBoard where kind = 3 and idx2 > 10
-- select top 10 * from dbo.tFVUserBoard where kind = 1 order by idx2 desc
exec spu_FVUserBoard 'xxxx2', '049000s1i0n7t8445289', 2, 1, 1, '', -1					-- 친추.
exec spu_FVUserBoard 'xxxx2', '049000s1i0n7t8445289', 2, 2, 1, '', -1					-- 일반.
exec spu_FVUserBoard 'xxxx2', '049000s1i0n7t8445289', 2, 3, 1, '', -1					-- 대항.

--글읽기 테스트.
exec spu_FVUserBoard 'xxxx2', '049000s1i0n7t8445289', 2,  1, 0, '', -1					-- 친추.
exec spu_FVUserBoard 'xxxx2', '049000s1i0n7t8445289', 2,  1, 1, '', -1					-- 친추.
exec spu_FVUserBoard 'xxxx2', '049000s1i0n7t8445289', 2,  1, 2, '', -1					-- 친추.
exec spu_FVUserBoard 'xxxx2', '049000s1i0n7t8445289', 2,  1, 7456, '', -1					-- 친추.
exec spu_FVUserBoard 'xxxx2', '049000s1i0n7t8445289', 2,  1, 7457, '', -1					-- 친추.
exec spu_FVUserBoard 'xxxx2', '049000s1i0n7t8445289', 2,  1, 7458, '', -1					-- 친추.
*/
use Farm
GO

IF OBJECT_ID ( 'dbo.spu_FVUserBoard', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FVUserBoard;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_FVUserBoard
...
Begin
	...
	set @gapminute 		= dbo.fnu_GetFVDatePart('mi', @boardwrite, getdate())
	...
	if(@gameid = '')
		...
		BEGIN
			...
			from dbo.tFVUserBoard where kind = @kind_
		...
		begin
			...
			if(@mode_ = @USERBOARD_MODE_WRITE)
				begin
					...
					select top 10 @pagemax pagemax, @page page, idx, idx2, kind, gameid, message, writedate, schoolidx from dbo.tFVUserBoard
			...
			select b.*, isnull(m.itemcode, 1) itemcode, isnull(acc1, -1) acc1, isnull(acc2, -1) acc2, kakaoprofile, kakaonickname from
				(select gameid, anirepitemcode itemcode, anirepacc1 acc1, anirepacc2 acc2, kakaoprofile, kakaonickname from dbo.tFVUserMaster where gameid in (select gameid from @tTempTableList)) as m
			JOIN
				@tTempTableList b
			ON
				m.gameid = b.gameid
