		...
		<td align="center">
			...
			<div  style="border:1px solid #D7D6D6;background:#FCFCFC;padding:36px 0;">
				<table>
					<tr>
						<td colspan=4>
							학교대항전 랭킹(<a href=schoolmaster_list.jsp>돌아가기</a>)
						</td>
					</tr>
				</table>
				<table border=1>
					<%
					//2. 데이타 조작
					//exec spu_FarmD 19, 94,  4,  1, -1, -1, -1, -1, -1, -1, '', '', '', '', '', '', '', '', '', ''						-- 학교대항 서브 리스트(학교)
					...
					<%while(result.next()){
						totalpoint = totalpoint + result.getInt("point");%>
						<tr>
							<td><%=idx++%></td>
							<td><%=result.getString("userrank")%></td>
							<td><%=result.getString("schoolname")%></td>
							<td><%=result.getString("gameid")%></td>
							<td><%=String.format("%,d", (long)result.getInt("point"))%></td>
							<td><%=result.getString("joindate")%></td>
							<td>
								<a href=usersetting_ok.jsp?p1=19&p2=94&p3=1&ps1=<%=result.getString("gameid")%>&ps2=<%=adminid%>&branch=userinfo_list&gameid=<%=result.getString("gameid")%> target=_blank>강제탈퇴처리</a>
								<a href=userinfo_list.jsp?gameid=<%=result.getString("gameid")%> target=_blank><%=result.getString("gameid")%></a><br>
							</td>
						</tr>
					<%}%>
					<tr>
						<td colspan=7><%=totalpoint%> </td>
					</tr>
				</table>
			</div>
			</form>
