			...
			<div  style="border:1px solid #D7D6D6;background:#FCFCFC;padding:36px 0;">
				...
				<table>
					<tr>
						<td>
							할인은 마스터에서 관리합니다.
								<font size=10>
									<a href=systemroul_list2.jsp?packstate=-1>삭제보기</a>
									<a href=systemroul_list2.jsp?packstate=1>정상보기</a>
								</font>
						</td>
					</tr>
				</table>
				...
				<table border=1>
					<tr>
						...
						<td></td>
						<td>상품</td>
						<td><a href=systemroul_list2.jsp?packstate=<%=packstate==1?-1:1%>><%=packstate==1?"삭제보기":"정상보기"%></a></td>
					</tr>
					<%if(cnt > 0){
						if(cstmt.getMoreResults()){
							result = cstmt.getResultSet();
							while(result.next()){
								...
								<tr <%=getCheckValueOri(result.getInt("packstate"), -1, "bgcolor=#aaaaaa", "")%> <%=getCheckValueOri(result.getInt("idx"), idx, "bgcolor=#ffe020", "")%> >
									...
									<input type="hidden" name="branch" value="systemroul_list2">
