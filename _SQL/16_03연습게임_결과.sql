...
IF OBJECT_ID ( 'dbo.spu_PTResult', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_PTResult;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_PTResult
...
as
	...
	-- 기타상수.
	declare @PRACTICE_GAME_LOG_MAX				int					set @PRACTICE_GAME_LOG_MAX 			= 15	-- 15개만 저장한다.
...
Begin
	...
	if(@cursyscheck = @SYSCHECK_YES)
		...
		BEGIN
			...
			--select 'DEBUG ' + @comment
		END
	else if ( exists( select top 1 * from dbo.tPracticeGameLog where gameid = @gameid_ and curturntime = @curturntime_ ) )
		BEGIN
			...
			where gameid = @gameid_ and curturntime = @curturntime_
	...
	-- 이하부터는 로또정보가 들어와 있음...(위에서 필터해줌)
		...
		BEGIN
			...
			------------------------------------------------
			select @idx2 = ( ISNULL( MAX( idx2 ), 0) + 1) from dbo.tPracticeGameLog where gameid = @gameid_
			...
			-- 배팅정보 삭제.
			------------------------------------------------
			--select 'DEBUG 2-3 배팅정보삭제', * from dbo.tPracticeGame where gameid = @gameid_ and curturntime = @curturntime
			delete from dbo.tPracticeGame where gameid = @gameid_ and curturntime = @curturntime
