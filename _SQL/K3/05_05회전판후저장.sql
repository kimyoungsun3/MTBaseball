...
Go
/*
update dbo.tFVUserMaster set randserial = -1, wheeldayfree = 1, ownercashcost = 10000, wheelgauage = 0, wheelfree = 1 where gameid = 'xxxx2'
exec spu_FVWheel 'xxxx2', '049000s1i0n7t8445289', 20,    0, 'savedata1', 7776, -1			-- 일일회전판(20)    MODE_WHEEL_NORMAL
exec spu_FVWheel 'xxxx2', '049000s1i0n7t8445289', 21, 1000, 'savedata2', 7777, -1			-- 결정회전판(21)    MODE_WHEEL_PREMINUM
exec spu_FVWheel 'xxxx2', '049000s1i0n7t8445289', 22,    0, 'savedata3', 7778, -1			-- 황금무료(22)  	 MODE_WHEEL_PREMINUMFREE
...
as
	...
	declare @wheeldayfree			int						set @wheeldayfree	= -1
...
Begin
	...
	--select 'DEBUG 입력정보', @gameid_ gameid_, @password_ password_, @mode_ mode_, @cashcost_ cashcost_, @savedata_ savedata_, @randserial_ randserial_
	...
	select
		...
		@ownercashcost	= ownercashcost,	@wheeldayfree	= wheeldayfree,
	...
	if(@gameid = '')
		...
		BEGIN
			...
			--select 'DEBUG ' + @comment
		END
	else if (@mode_ = @MODE_WHEEL_NORMAL and @wheeldayfree = -1)
		BEGIN
		...
		BEGIN
			...
			if(@mode_ = @MODE_WHEEL_NORMAL)
				begin
					--select 'DEBUG 일일회전판'
					exec spu_FVDayLogInfoStatic @market, 63, 1			-- 일 일일회전판
					set @wheeldayfree = -1
				end
				...
				begin
					...
					exec spu_FVDayLogInfoStatic @market, 61, 1			-- 일 결정회전판
					...
					if(@ownercashcost2 > @ownercashcost )
						begin
							set @strange = 1
							set @comment2 = '결정회전판 보유(' + ltrim(rtrim(@ownercashcost)) + ') < 현재비용(' + ltrim(rtrim(@ownercashcost2)) + ')'
							--select 'DEBUG **** 이상기록', @comment2 comment2
							exec spu_FVSubUnusualRecord2  @gameid_, @comment2
						end
				end
				...
				begin
					--select 'DEBUG 황금무료'
					exec spu_FVDayLogInfoStatic @market, 62, 1			-- 일 황금무료
			...
			-- 유저 정보 갱신
			----로그인
			--	아직안될림  	: <wheeldayfree>1</wheeldayfree>
			--	돌림			: <wheeldayfree>-1</wheeldayfree>
			--
			----일반 회전판 돌림.
	...
	-- 결과코드처리.
	-----------------------------------------------
	select @nResult_ rtn, @comment comment, @wheeldayfree wheeldayfree, @wheelgauageflag wheelgauageflag, @wheelgauage wheelgauage, @wheelfree wheelfree
