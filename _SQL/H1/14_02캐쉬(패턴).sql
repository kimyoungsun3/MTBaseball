...
Begin
	...
	if(@market_ in (@MARKET_GOOGLE, @MARKET_GOOGLE2))
		begin
			...
			if(not exists(select top 1 * from dbo.tUserGoogleBuyLog where gameid = @gameid_))
				begin
					--select 'DEBUG (구글 패턴분석) 존재안함 > 첫로그기록'
					insert into dbo.tUserGoogleBuyLog(gameid, buypoint) values(@gameid_, @cashpoint * @cashpoint)
				end
			else
				begin
					--select 'DEBUG (구글 패턴분석) 존재함 > 분석후 > 블럭처리유무파악'
					...
					set @buypointplus = CASE
											WHEN (@buypointmin <= 0) then 400
											WHEN (@buypointmin <= 2) then 300
											WHEN (@buypointmin <= 4) then 250
											WHEN (@buypointmin <= 6) then 200
											WHEN (@buypointmin <= 8) then 100
											WHEN (@buypointmin <= 10) then 60
											WHEN (@buypointmin <= 20) then 40
											WHEN (@buypointmin <= 40) then 20
											WHEN (@buypointmin <= 60) then 10
											WHEN (@buypointmin <= 90) then 5
											ELSE -1000
										END
					set @buypoint = @buypoint + @cashpoint * @buypointplus
					set @buypoint = case when @buypoint < 0 then 0 else @buypoint end
					--select 'DEBUG ', @cashpoint cashpoint, @buypointplus buypointplus, @buypoint buypoint

					update dbo.tUserGoogleBuyLog
						set
							buypoint = @buypoint
					...
					if(@buypoint > 19000)
						begin
							...
							if(LEN(@phone) > 5)
								begin
									...
									if(not exists(select top 1   * from dbo.tUserBlockPhone where phone = @phone))
										begin
											insert into dbo.tUserBlockPhone(phone, comment) values(@phone, '(구글자동)캐쉬치트해서 일괄블럭함')

											insert into dbo.tUserBlockLog(gameid, comment)
											values(@gameid_, '(구글자동)직접 블럭 처리했습니다.')
										end
								end
							else
								begin
							...
							if(isnull(@giftid_, '') != '')
								begin
									-- 2-1. 커서 설정
									declare curCashLogGift Cursor for
									select Convert(varchar(8), writedate, 112), market, goldball, cash, idx from dbo.tCashLog where gameid = @giftid_

									-- 2-2. 커서오픈
									open curCashLogGift

									-- 2-3. 커서 사용
									Fetch next from curCashLogGift into @dateid, @market, @goldball, @cash, @idx
									while @@Fetch_status = 0
										Begin
											-- 로그삭제
											delete dbo.tCashLog where idx = @idx and gameid = @giftid_
									...
									-- 2-4. 커서닫기
									close curCashLogGift
									Deallocate curCashLogGift
								end
