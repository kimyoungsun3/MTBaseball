...
exec spu_FVCashBuy3 'xxxx2', '049000s1i0n7t8445289', '',      '7000', 'googlekw', '12999763169054705758.1343569877497000', '', '', -1	-- 친구없음
exec spu_FVCashBuy3 'xxxx2', '049000s1i0n7t8445289', 'xxxx3', '7010', 'googlekw', '12999763169054705758.1343569877497010', '', '', -1	-- 친구있음
...
exec spu_FVCashBuy3 'xxxx2', '049000s1i0n7t8445289', '',      '7005', 'real', 'error', '암호문', '해석문', -1					-- IPHONE(real)
...
Begin
	...
	if(@gameid = '')
		...
		BEGIN
			...
			select @nResult_ rtn, '블럭처리된 아이디입니다.(1)'
		END
		...
		begin
			...
			if(@cashcopy >= 0)
				begin
					set @nResult_ 	= @RESULT_ERROR_BLOCK_USER
					select @nResult_ rtn, '블럭처리된 아이디입니다.(2)'
					...
					values(@gameid_, '(캐쉬결재)를  '+ltrim(rtrim(str(@cashcopy)))+'회 이상 카피를 해서 캐쉬입력에서 시스템에서 블럭 처리했습니다.')
				end
			else
				begin
			...
			insert into dbo.tFVUserUnusualLog(gameid, comment) values(@gameid_, '**** 유효하지 않는 캐쉬코드가 입력되었습니다. ****')
		end
