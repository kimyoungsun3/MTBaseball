...
--exec spu_FVFarmD 19, 89, 24, 156, -1, -1, -1, -1, -1, -1, 'xxxx2', 'admin', '', '', '', '', '', '', '', ''				-- 개발 삭제.
--exec spu_FVFarmD 19, 89, 24, 156, -1, -1, -1, -1, -1, -1, 'xxxx2', 'admin', '', '', '', '', '', '', '', ''				-- 개발 삭제.
...
--exec spu_FVFarmD 17, 3, 22, 5000, -1, -1, -1, -1, -1, -1, 'xxxx2', '', '', 'acodedd', '2015-01-08', '', '', '', '', ''		-- 수정
...
--exec spu_FVFarmD 30, 12, -1,500,504, 69, 10,  1, -1, -1, '바나나 패키지','내용', '1:80214;2:3500;3:3300;4:-1;5:-1;6:1;7:2;8:3;9:0;10:0;', '1,5,6,7', '', '', '', '', '', ''
--exec spu_FVFarmD 30, 13,  1,500,540, 69, 10,  1, -1, -1, '바나나 패키지','내용', '1:80214;2:3500;3:3300;4:-1;5:-1;6:1;7:2;8:3;9:0;10:0;', '1,5,6,7', '', '', '', '', '', ''
...
as
	...
	-- 복귀플래그.
	declare @rtnflag			int					set @rtnflag			= -1
Begin
	------------------------------------------------
	--	초기화
	------------------------------------------------
		...
		begin
			if(isnull(@ps2_, '') != '')
				...
				begin
					...
					---- 환생.
					-----------------------------------------------
					select top 10 * from dbo.tFVRebirthLog
					where gameid = @gameid
					order by idx desc
			...
			if(@p2_ = 1)
				...
				begin
					...
					if(isnull(@gameid, '') = '')
						begin
							set @idxPage	= @p7_
							select @idx = (isnull(max(idx), 1)) from dbo.tFVRebirthLog
							...
							select top 100 @maxPage maxPage, @page page, * from dbo.tFVRebirthLog
							where idx <= @idx order by idx desc
						end
					else
						begin
				...
				begin
					if(@p3_ = 1)
						...
						begin
							update dbo.tFVUserData set savedate = savedate - 0.0105 where gameid = @gameid
						end
					else if(@p3_ = 22)
						begin
							update dbo.tFVUserData set savedate = savedate - 1 where gameid = @gameid
						end
					select 1 rtn
				end
			else if(@p2_ = 89)
				begin
					if(@p3_ = 1)
						...
						begin
							update dbo.tFVUserMaster set rtndate = rtndate - 1, condate = condate - 1 where gameid = @gameid
						end
					else if(@p3_ = 4)
						begin
							update dbo.tFVUserMaster set rtndate = rtndate - 30, condate = condate - 31 where gameid = @gameid
						end
					else if(@p3_ = 24)
						begin
							--select 'DEBUG 24'
							delete from dbo.tFVRebirthLog where gameid = @gameid and idx = @p4_
						end
					else if(@p3_ = 25)
						begin
							--select 'DEBUG 25'
							delete from dbo.tFVRebirthLog where gameid = @gameid
						end
					select 1 rtn
				end
			else if(@p2_ = 94)
			...
			-----------------------------------
				...
				begin
					if(@p3_ = 1)
						...
						begin
							select itemcode, itemcodename, SUM(tsgrade1cnt + tsgrade2cnt + tsgrade3cnt + tsgrade4cnt + tsgrade2freecnt + tsgrade3freecnt + tsgrade4freecnt) cnt from dbo.tFVRouletteLogTotalSub
	...
	-----------------------------------------------------
		...
		begin
			...
			if(@p2_ = 1)
				...
				begin
					if(not exists(select top 1 * from dbo.tFVCashLog where acode = @ps4_ and idx != @p3_))
						begin
				...
				begin
					select * from dbo.tFVCashLog where idx = @p3_
				end
		...
		begin
			if(@p2_ in (5, 6))
				begin
					...
					while @@Fetch_status = 0
						Begin
							...
							else if(@packkind = 30) set @rtnflag			= @packvalue
				...
				begin
					...
					where subcategory in (30, 31, 32, 33, 34, 35, 36, 37, 60, 80)
				...
				begin
					...
					if(not exists(select top 1 * from dbo.tFVSystemPack where packstr = @ps3_))
						begin
							...
							set @itemcode = 50000
							select @itemcode = isnull(max(itemcode), 50000)  + 1 from dbo.tFVItemInfo where subcategory = 500
							...
							if(not exists(select top 1 * from dbo.tFVItemInfo where itemcode = @itemcode))
								begin
									...
									insert into dbo.tFVSystemPack(itemcode, famelvmin, famelvmax, cashcostcost, cashcostper,             cashcostsale, packstate, packname, comment, packstr,  pack1,  pack2,  pack3,  pack4,  pack5, pack1cnt, pack2cnt, pack3cnt, pack4cnt, pack5cnt)
														  values(@itemcode,      @p4_,      @p5_,         @p6_,        @p7_, @p6_ - (@p6_ * @p7_)/100,      @p8_,    @ps1_,   @ps2_,   @ps3_, @pack1, @pack2, @pack3, @pack4, @pack5,   @pack6,   @pack7,   @pack8,   @pack9,  @pack10)
								end
						end
				...
				begin
					...
					update dbo.tFVSystemPack
							set
								...
								pack1cnt= @pack6,  		pack2cnt= @pack7,	pack3cnt= @pack8,	pack4cnt= @pack9,		pack5cnt= @pack10
					where idx = @p3_
				...
				begin
					...
					select top 50 * from Game4FarmVill4.dbo.tFVUserPushSendInfo order by idx desc
				end
				...
				begin
					...
					select @recepushid = pushid, @market = market from Game4FarmVill4.dbo.tFVUserMaster
					where gameid = @gameid
				...
				begin
					...
					if(@p4_ = @IPHONE)
						begin
								...
								begin
									insert into Farm.dbo.tUserPushiPhone(recepushid,   sendid, receid, sendkind, msgpush_id, msgtitle, msgmsg, msgaction)
										select distinct                      pushid, @adminid, gameid,     @p3_,         99,    @ps3_,  @ps4_,     @ps5_ from Farm.dbo.tUserMaster
											...
											and phone not in (select phone from Game4FarmVill4.dbo.tFVPushBlackList)
								end
							else if(@p5_ = 2)
								begin
							...
							--	메세지 내용기록
							-----------------------------
							insert into Game4FarmVill4.dbo.tFVUserPushSendInfo(adminid,  sendkind, market, msgtitle, msgmsg,  cnt, msgurl)
					...
					if(@p4_ = @IPHONE)
						begin
							...
							if(@p5_ = 1)
								begin
									insert into Farm.dbo.tUserPushiPhone(recepushid,   sendid, receid,        sendkind, msgpush_id, msgtitle, msgmsg, msgaction)
										select distinct                      pushid, @adminid, rtrim(gameid),     @p3_,         99,    @ps3_,  @ps4_,     @ps5_ from Game4FarmVill4.dbo.tFVUserMaster
	...
	-----------------------------------------------------
		...
		begin
			...
			if(@subkind	= 1)
				begin
					select * from dbo.tFVItemInfo where subcategory in (30, 31, 32, 33, 34, 35, 36, 37, 60, 80) and itemcode != 3015
					union all
