...
create table dbo.tFVUserMaster(
	...
	-- 환생.
	rebirthcnt		int					default(0),				-- 환생 횟수.
	rebirthpoint	int					default(0),				-- 환생포인트.
	rebirthdate		datetime			default(getdate()),
	...
	eventspot10		int					default(0),				-- 환생별에대한 보물 지급.
	eventspot11		int					default(0),				-- 환생 특별 이벤트.
	eventspot12		int					default(0),				-- 2만명 달성 루비지급.
	eventspot13		int					default(0),				-- 미정.
...
create table dbo.tFVUserData(
	...
	savedata	varchar(8000)			default('-1'),
...
create table dbo.tFVCashLog(
	...
	idata			varchar(8000),							-- 아이폰에서 사용되는 데이타(원본)
	idata2			varchar(8000),							-- 아이폰에서 사용되는 데이타(해석본)
	concode			int				default(82),			-- 한국(82), 일본(81), 미국(1)
...
--		레벨업리워드.
---------------------------------------------
IF OBJECT_ID (N'dbo.tFVRebirthLog', N'U') IS NOT NULL
	DROP TABLE dbo.tFVRebirthLog;
GO

create table dbo.tFVRebirthLog(
	idx			int 					IDENTITY(1, 1),			-- indexing
	...
	-- Constraint
	CONSTRAINT pk_tFVRebirthLog_idx	PRIMARY KEY(idx)
)
GO

IF EXISTS (SELECT name FROM sys.indexes WHERE name = N'idx_tFVRebirthLog_gameid_idx')
    DROP INDEX tFVRebirthLog.idx_tFVRebirthLog_gameid_idx
GO
CREATE INDEX idx_tFVRebirthLog_gameid_idx ON tFVRebirthLog (gameid, idx)
GO