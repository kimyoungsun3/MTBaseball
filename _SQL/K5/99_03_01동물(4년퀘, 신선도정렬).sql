...
if(@mode = 1)
	begin
		...
		-- 1. 선언하기.
		declare curGameMTBaseballOrder Cursor for
		select idx from @tItemExpire3 order by itemcode asc

		-- 2. 커서오픈
		open curGameMTBaseballOrder

		-- 3. 커서 사용
		Fetch next from curGameMTBaseballOrder into @idx
		...
		-- 4. 커서닫기
		close curGameMTBaseballOrder
		Deallocate curGameMTBaseballOrder
		...
		-- 1. 선언하기.
		declare curAnimalToGameMTBaseball Cursor for
		select idx, grade, itemcode, subcategory from @tItemExpire order by fresh asc
		--select 'DEBUG ', * from @tItemExpire order by idx asc

		-- 2. 커서오픈
		open curAnimalToGameMTBaseball

		-- 3. 커서 사용
		Fetch next from curAnimalToGameMTBaseball into @idx, @grade, @itemcode, @subcategory
		while @@Fetch_status = 0
			Begin
		...
		-- 4. 커서닫기
		close curAnimalToGameMTBaseball
		Deallocate curAnimalToGameMTBaseball
