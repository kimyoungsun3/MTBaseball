...
update dbo.tUserMaster set cashcost = 100000, gamecost = 100000, heart = 100000, cashpoint = 140000, randserial = -1 where gameid = 'xxxx2'

exec spu_RoulBuyNew 'xxxx2', '049000s1i0n7t8445289', 1, 'xxxx3', 7721, -1			-- 일반뽑기
exec spu_RoulBuyNew 'xxxx2', '049000s1i0n7t8445289', 1, 'xxxx3', 7722, -1
exec spu_RoulBuyNew 'xxxx2', '049000s1i0n7t8445289', 2, 'xxxx3', 7723, -1			-- 루비뽑기
exec spu_RoulBuyNew 'xxxx2', '049000s1i0n7t8445289', 2, 'xxxx3', 7724, -1
exec spu_RoulBuyNew 'xxxx2', '049000s1i0n7t8445289', 4, 'xxxx3', 7725, -1			-- 루비뽑기 10 + 1
exec spu_RoulBuyNew 'xxxx2', '049000s1i0n7t8445289', 4, 'xxxx3', 7726, -1

update dbo.tUserMaster set anigrade2gauage = 100, randserial = -1 where gameid = 'xxxx2'
exec spu_RoulBuyNew 'xxxx2', '049000s1i0n7t8445289', 2, 'xxxx3', 7774, -1			-- 루비뽑기				(무료).
update dbo.tUserMaster set anigrade4gauage = 100, randserial = -1 where gameid = 'xxxx2'
exec spu_RoulBuyNew 'xxxx2', '049000s1i0n7t8445289', 4, 'xxxx3', 7775, -1			-- 루비뽑기 10 + 1		(무료).

exec spu_SetDirectItem 'xxxx2',   2200, 1, -1		update dbo.tUserMaster set randserial = -1 where gameid = 'xxxx2'
exec spu_RoulBuyNew 'xxxx2', '049000s1i0n7t8445289', 1, 'xxxx3', 7776, -1			-- 일반뽑기				(티켓).
exec spu_SetDirectItem 'xxxx2',   2300, 1, -1		update dbo.tUserMaster set randserial = -1 where gameid = 'xxxx2'
exec spu_RoulBuyNew 'xxxx2', '049000s1i0n7t8445289', 2, 'xxxx3', 7777, -1			-- 루비뽑기				(티켓).
-- 없음.																			-- 루비뽑기 10 + 1		(티켓).
*/
use Game4Farmvill5
GO

IF OBJECT_ID ( 'dbo.spu_RoulBuyNew', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_RoulBuyNew;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_RoulBuyNew
	...
	if(@gameid = '')
		...
		BEGIN
			...
			if(@pmgauageflag = 1 and @mode_ in ( @MODE_ROULETTE_GRADE2, 		@MODE_ROULETTE_GRADE4,
				...
				begin
					set @vip_plus = dbo.fun_GetVIPPlus( 4, @cashpoint, 0)		-- 교배.
				...
				begin
					...
					if( @mode_ = @MODE_ROULETTE_GRADE2_FREE )
						begin
							...
							set @anigrade2gauage	= @anigrade2gauage - @PMGAUAGEMAX
						end
				...
				begin
					...
					if( @mode_ = @MODE_ROULETTE_GRADE4_FREE )
						begin
							...
							set @anigrade4gauage	= @anigrade4gauage - @PMGAUAGEMAX
						end
			...
			-- 템을 보정해둔다.
			--------------------------------------------
			if( @mode_ in ( @MODE_ROULETTE_GRADE1, @MODE_ROULETTE_GRADE1_TICKET ) and @anigrade1cnt in (1, 3, 5, 10, 25) )
				begin
			...
			else if( @mode_ in ( @MODE_ROULETTE_GRADE4_FREE ) )
				begin
					-- 10 + 1 -> 무료상태에서는 반드시 4등급이 나오도록한다.
					select @roul1 = itemcode from dbo.tItemInfo
					where category = @ITEM_MAINCATEGORY_ANI
						  and grade in ( 5 )
						  and houselv <= @OPEN_STEP
			...
			else if( @mode_ in ( @MODE_ROULETTE_GRADE4 ) and @anigrade4cnt = 1 )
				begin
					-- 처음 10 + 1 -> 반드시 4등급이 나오도록한다.
					select @roul1 = itemcode from dbo.tItemInfo
					where category = @ITEM_MAINCATEGORY_ANI
						  and grade in ( 4 )
						  and houselv <= @OPEN_STEP
			...
			-- 뽑기 정보 셔플하기.
			------------------------------------------------------------------
			--select 'DEBUG 뽑기(셔플전)', @roul1 roul1, @roul2 roul2, @roul3 roul3, @roul4 roul4, @roul5 roul5, @roul6 roul6, @roul7 roul7, @roul8 roul8, @roul9 roul9, @roul10 roul10, @roul11 roul11
			set @rand 	= Convert(int, ceiling(RAND() * 100))
			select
				@roul1 = itemcode1,
				@roul2 = itemcode2,
				...
				@roul10 = itemcode10,
				@roul11 = itemcode11
			from dbo.fnu_GetShuffleCard(@rand, 	@roul1, @roul2, @roul3, @roul4, @roul5, @roul6, @roul7, @roul8, @roul9, @roul10, @roul11)
			--select 'DEBUG 뽑기(셔플후)', @roul1 roul1, @roul2 roul2, @roul3 roul3, @roul4 roul4, @roul5 roul5, @roul6 roul6, @roul7 roul7, @roul8 roul8, @roul9 roul9, @roul10 roul10, @roul11 roul11
			...
			if(@roulflag = 1)
				begin
					...
					--select 'DEBUG 당첨동물개수.', @roulani1cnt roulani1cnt, @roulani2cnt roulani2cnt, @roulani3cnt roulani3cnt
					...
					if( @roulani1cnt > 0 )
						begin
							...
							--select 'DEBUG > 	-> 특정동물 보상1', @checkani checkani, @roulname roulname, @checkreward checkreward, @checkrewardcnt checkrewardcnt
							while( @roulanicnt > 0 )
								begin
					...
					if( @roulani2cnt > 0 )
						begin
							...
							--select 'DEBUG > 	-> 특정동물 보상2', @checkani checkani, @roulname roulname, @checkreward checkreward, @checkrewardcnt checkrewardcnt
							while( @roulanicnt > 0 )
								begin
									exec spu_SubGiftSendNew @GIFTLIST_GIFT_KIND_GIFT,  @checkreward, @checkrewardcnt, @roulname, @gameid_, ''
									set @roulanicnt = @roulanicnt - 1
					...
					if( @roulani3cnt > 0 )
						begin
							...
							--select 'DEBUG > 	-> 특정동물 보상3', @checkani checkani, @roulname roulname, @checkreward checkreward, @checkrewardcnt checkrewardcnt
							while( @roulanicnt > 0 )
								begin
									exec spu_SubGiftSendNew @GIFTLIST_GIFT_KIND_GIFT,  @checkreward, @checkrewardcnt, @roulname, @gameid_, ''
									set @roulanicnt = @roulanicnt - 1
