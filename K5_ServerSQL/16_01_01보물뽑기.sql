...
update dbo.tUserMaster set tsgrade1cnt = 0,   tsgrade2cnt = 0,   tsgrade4cnt = 0, tsgrade2gauage = 0, tsgrade4gauage = 0, randserial = -1 where gameid = 'xxxx2'
update dbo.tUserMaster set cashcost = 1000000, gamecost = 0,      heart = 0,      randserial = -1, cashpoint = 0      where gameid = 'xxxx2'
update dbo.tUserMaster set cashcost = 1000000, gamecost = 100000, heart = 140000, randserial = -1, cashpoint = 140000 where gameid = 'xxxx2'

exec spu_RoulTreasureBuyNew 'xxxx2', '049000s1i0n7t8445289', 1, 7721, -1			-- 일반뽑기
exec spu_RoulTreasureBuyNew 'xxxx2', '049000s1i0n7t8445289', 1, 7722, -1
exec spu_RoulTreasureBuyNew 'xxxx2', '049000s1i0n7t8445289', 2, 7723, -1			-- 루비뽑기
exec spu_RoulTreasureBuyNew 'xxxx2', '049000s1i0n7t8445289', 2, 7724, -1
exec spu_RoulTreasureBuyNew 'xxxx2', '049000s1i0n7t8445289', 4, 7725, -1			-- 루비뽑기 10 + 1
exec spu_RoulTreasureBuyNew 'xxxx2', '049000s1i0n7t8445289', 4, 7726, -1

update dbo.tUserMaster set tsgrade2gauage = 100, randserial = -1 where gameid = 'xxxx2'
exec spu_RoulTreasureBuyNew 'xxxx2', '049000s1i0n7t8445289', 2, 7774, -1			-- 루비뽑기				(무료).
update dbo.tUserMaster set tsgrade4gauage = 100, randserial = -1 where gameid = 'xxxx2'
exec spu_RoulTreasureBuyNew 'xxxx2', '049000s1i0n7t8445289', 4, 7775, -1			-- 루비뽑기 10 + 1		(무료).

exec spu_SetDirectItem 'xxxx2',   2500, 1, -1		update dbo.tUserMaster set randserial = -1 where gameid = 'xxxx2'
exec spu_RoulTreasureBuyNew 'xxxx2', '049000s1i0n7t8445289', 1, 7776, -1			-- 일반뽑기				(티켓).
exec spu_SetDirectItem 'xxxx2',   2600, 1, -1		update dbo.tUserMaster set randserial = -1 where gameid = 'xxxx2'
exec spu_RoulTreasureBuyNew 'xxxx2', '049000s1i0n7t8445289', 2, 7777, -1			-- 루비뽑기				(티켓).
-- 없음.																			-- 루비뽑기 10 + 1		(티켓).
*/
use GameMTBaseball
GO

IF OBJECT_ID ( 'dbo.spu_RoulTreasureBuyNew', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_RoulTreasureBuyNew;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_RoulTreasureBuyNew
...
as
	...
	declare @inventreasuremax		int				set @inventreasuremax	= 0
	...
	declare @roulname				varchar(20)		set @roulname			= ''
	declare @roulani1cnt			int				set @roulani1cnt		= 0
	declare @roulani2cnt			int				set @roulani2cnt		= 0
	declare @roulani3cnt			int				set @roulani3cnt		= 0
	declare @roulanicnt				int				set @roulanicnt			= 0
	...
	DECLARE @tTempEventResultTable TABLE(
		checkani		int,
		checkreward		int,
		checkrewardcnt	int
	);
Begin
	...
	if( @gameid != '' )
		begin
			...
			where gameid = @gameid_
				  and invenkind = @ITEM_MAINCATEGORY_TREASURE
			--select 'DEBUG 4-2 인벤수량(전)', @invencnt invencnt, @inventreasuremax inventreasuremax
			set @invencnt = @invencnt + case
												when @mode_ in (@MODE_TREASURE_GRADE1,                             @MODE_TREASURE_GRADE1_TICKET) then 1
												when @mode_ in (@MODE_TREASURE_GRADE2, @MODE_TREASURE_GRADE2_FREE, @MODE_TREASURE_GRADE2_TICKET) then 1
												--when @mode_ in (@MODE_TREASURE_GRADE4, @MODE_TREASURE_GRADE4_FREE, @MODE_TREASURE_GRADE4_TICKET) then 11
												else																							      11
										  end
			--select 'DEBUG 4-2 인벤수량(후)', @invencnt invencnt, @inventreasuremax inventreasuremax
		end
	...
	if(@gameid = '')
		...
		END
	else if(@invencnt > @inventreasuremax)
		begin
			set @nResult_ = @RESULT_ERROR_INVEN_FULL
			set @comment = 'ERROR 보물 인벤이 풀입니다.'
			--select 'DEBUG ' + @comment, @invencnt invencnt, @inventreasuremax inventreasuremax
		end
	else if (@itemcode = -1)
		BEGIN
		...
		BEGIN
			...
			if(@pmgauageflag = 1 and @mode_ in ( @MODE_TREASURE_GRADE2, 		@MODE_TREASURE_GRADE4,
				...
				begin
					set @vip_plus = dbo.fun_GetVIPPlus( 6, @cashpoint,   0)	-- 보물.
					set @PMGAUAGEPOINT = @PMGAUAGEPOINT + @vip_plus
			...
			if( @mode_ in ( @MODE_TREASURE_GRADE1, @MODE_TREASURE_GRADE1_TICKET ) )
				begin
					set @group1 	= dbo.fun_GetTreasureGroupPercent( @mode_, 1, @cashpoint)
					set @group2		= dbo.fun_GetTreasureGroupPercent( @mode_, 2, @cashpoint)
					set @group3		= dbo.fun_GetTreasureGroupPercent( @mode_, 3, @cashpoint)
					set @group4		= dbo.fun_GetTreasureGroupPercent( @mode_, 4, @cashpoint)
					set @group5		= dbo.fun_GetTreasureGroupPercent( @mode_, 5, @cashpoint)
					set @sendid		= 'SysRoul1'
					set @cnt		= 1
					...
					exec spu_subRankDaJun @gameid_, 0, 0, 0, 1, 0, 0, 0		-- 교배,보물포인트
				...
				begin
					...
					if( @mode_ = @MODE_TREASURE_GRADE2_FREE )
						begin
							...
							set @tsgrade2gauage	= @tsgrade2gauage - @PMGAUAGEMAX
						end
				...
				begin
					...
					exec spu_UserItemBuyLogNew @gameid_, @itemcode, @needgamecost, @needcashcost, 0
					exec spu_subRankDaJun @gameid_, 0, 0, 0, 10, 0, 0, 0		-- 교배,보물포인트
					...
					if( @mode_ = @MODE_TREASURE_GRADE4_FREE )
						begin
							...
							set @tsgrade4gauage	= @tsgrade4gauage - @PMGAUAGEMAX
						end
					--else if( @mode_ = @MODE_TREASURE_GRADE4_TICKET )
					--	begin
					--	end
				end
			--select 'DEBUG ', @sendid sendid, @cnt cnt, @tsgrade2cnt tsgrade2cnt, @group1 group1, @group2 group2, @group3 group3, @group4 group4, @group5 group5
			...
			--select 'DEBUG ', @rand rand, @rand2 rand2
			...
			else if( @mode_ in ( @MODE_TREASURE_GRADE4_FREE ) )
				begin
					-- 10 + 1 -> 무료상태에서는 반드시 SS등급이 나오도록한다.
					select @roul1 = itemcode from dbo.tItemInfo
					where category = @ITEM_MAINCATEGORY_TREASURE
						  and grade in ( 6 )
						  and houselv <= @OPEN_STEP
			...
			if(@roulflag = 1)
				begin
					--select 'DEBUG > 이벤트 기간내', @roulani1 roulani1, @roulani2 roulani2, @roulani3 roulani3

					insert into @tTempTable(roul) values( @roul1  )
					insert into @tTempTable(roul) values( @roul2  )
					insert into @tTempTable(roul) values( @roul3  )
					insert into @tTempTable(roul) values( @roul4  )
					insert into @tTempTable(roul) values( @roul5  )
					insert into @tTempTable(roul) values( @roul6  )
					insert into @tTempTable(roul) values( @roul7  )
					insert into @tTempTable(roul) values( @roul8  )
					insert into @tTempTable(roul) values( @roul9  )
					insert into @tTempTable(roul) values( @roul10 )
					insert into @tTempTable(roul) values( @roul11 )

					select @roulani1cnt = count(*) from @tTempTable where roul = @roulani1
					select @roulani2cnt = count(*) from @tTempTable where roul = @roulani2
					select @roulani3cnt = count(*) from @tTempTable where roul = @roulani3
					--select 'DEBUG 당첨개수.', @roulani1cnt roulani1cnt, @roulani2cnt roulani2cnt, @roulani3cnt roulani3cnt

					if( @roulani1cnt > 0 )
						begin
							set @checkani 		= @roulani1
							set @roulname		= @roulname1
							set @checkreward 	= @roulreward1
							set @checkrewardcnt	= @roulrewardcnt1
							set @roulanicnt		= @roulani1cnt
							--select 'DEBUG > 	-> 특정 보상1', @checkani checkani, @roulname roulname, @checkreward checkreward, @checkrewardcnt checkrewardcnt
							while( @roulanicnt > 0 )
								begin
									exec spu_SubGiftSendNew @GIFTLIST_GIFT_KIND_GIFT,  @checkreward, @checkrewardcnt, @roulname, @gameid_, ''
									set @roulanicnt = @roulanicnt - 1

									insert into @tTempEventResultTable( checkani,  checkreward,  checkrewardcnt )
									values(                           @checkani, @checkreward, @checkrewardcnt )
								end
						end
					...
					if( @roulani2cnt > 0 )
						begin
							...
							set @roulname		= @roulname2
							set @checkreward 	= @roulreward2
							set @checkrewardcnt	= @roulrewardcnt2
							set @roulanicnt		= @roulani2cnt
							--select 'DEBUG > 	-> 특정 보상2', @checkani checkani, @roulname roulname, @checkreward checkreward, @checkrewardcnt checkrewardcnt
							while( @roulanicnt > 0 )
								begin
									exec spu_SubGiftSendNew @GIFTLIST_GIFT_KIND_GIFT,  @checkreward, @checkrewardcnt, @roulname, @gameid_, ''
									set @roulanicnt = @roulanicnt - 1
					...
					if( @roulani3cnt > 0 )
						begin
							...
							set @roulname		= @roulname3
							set @checkreward 	= @roulreward3
							set @checkrewardcnt	= @roulrewardcnt3
							set @roulanicnt		= @roulani3cnt
							--select 'DEBUG > 	-> 특정 보상3', @checkani checkani, @roulname roulname, @checkreward checkreward, @checkrewardcnt checkrewardcnt
							while( @roulanicnt > 0 )
								begin
									exec spu_SubGiftSendNew @GIFTLIST_GIFT_KIND_GIFT,  @checkreward, @checkrewardcnt, @roulname, @gameid_, ''
									set @roulanicnt = @roulanicnt - 1
		...
		begin
			...
			update dbo.tUserMaster
				set
					...
					tsgrade1cnt= @tsgrade1cnt,		tsgrade2cnt	= @tsgrade2cnt,tsgrade4cnt= @tsgrade4cnt,
													tsgrade2gauage	= @tsgrade2gauage,	tsgrade4gauage	= @tsgrade4gauage,
					bktsgrade1cnt= bktsgrade1cnt + case when ( @mode_ in ( @MODE_TREASURE_GRADE1,                              @MODE_TREASURE_GRADE1_TICKET ) ) then 1 else 0 end,
					bktsgrade2cnt= bktsgrade2cnt + case when ( @mode_ in ( @MODE_TREASURE_GRADE2, @MODE_TREASURE_GRADE2_FREE , @MODE_TREASURE_GRADE2_TICKET, @MODE_TREASURE_GRADE4, @MODE_TREASURE_GRADE4_FREE , @MODE_TREASURE_GRADE4_TICKET ) ) then 1 else 0 end
			where gameid = @gameid_
			...
			-- 이벤트결과.
			---------------------------------------------
			select * from @tTempEventResultTable
		end
