-----------------------------------------------------------------------
-- exec sup_subAddKakaoFriend 'xxxx2', '0:kakaouseridxxxx;1:kakaouseridxxxx3;'
-- exec sup_subAddKakaoFriend 'xxxx2', '0:x1;1:x2;'
...
IF OBJECT_ID ( 'dbo.sup_subAddKakaoFriend', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.sup_subAddKakaoFriend;
GO

create procedure dbo.sup_subAddKakaoFriend
	@gameid_				varchar(20),
	...
	if(LEN(@kakaouseridlist_) >= 3)
		begin
			...
			while @@Fetch_status = 0
				Begin
					set @friendid		= ''
					select @friendid = gameid from dbo.tKakaoMaster where kakaouserid = @kakaouserid
					...
					if(@friendid != '')
						begin
							...
							-- 내 - 친구
							if(not exists(select top 1 * from dbo.tUserFriend where gameid = @gameid_ and friendid = @friendid))
								begin
									--select 'DEBUG 내 - 친구(입력)'
									insert into dbo.tUserFriend(gameid,   friendid,  state,					   kakaofriendkind,         helpdate,      senddate)
							...
							--select 'DEBUG ', kakaouserid from dbo.tKakaoMaster where gameid = @gameid_
							--select 'DEBUG ', gameid from dbo.tUserMaster where kakaouserid in (select kakaouserid from dbo.tKakaoMaster where gameid = @gameid_) and kakaostatus != @KAKAO_STATUS_NEWSTART
							delete from dbo.tUserFriend
							where gameid = @friendid
								  and friendid in (select gameid from dbo.tUserMaster where kakaouserid in (select kakaouserid from dbo.tKakaoMaster where gameid = @gameid_) and kakaostatus != @KAKAO_STATUS_NEWSTART)

							if(not exists(select top 1 * from dbo.tUserFriend where gameid = @friendid and friendid = @gameid_))
								begin
