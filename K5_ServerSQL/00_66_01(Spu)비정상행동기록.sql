/*
update dbo.tUserMaster set resultcopy= 0, cashcopy = 0, blockstate = 0 where gameid = 'xxxx2'
...
IF OBJECT_ID ( 'dbo.spu_SubUnusualRecord', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_SubUnusualRecord;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_SubUnusualRecord
...
Begin
	...
	if(@resultcopy != -444 and @resultcopy_ >= 0)
		BEGIN
			...
			insert into dbo.tUserUnusualLog(gameid, comment) values(@gameid_, @comment_)
		END
