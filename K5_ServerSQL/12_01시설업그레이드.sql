...
-----------------------------------------------------
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445280', 60, 1, -1		-- 유저없음.
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', -1, 1, -1		-- 업글없음.
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 60,-1, -1		-- 단계없음.
...
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 61, 2, -1		--			(즉시완료)
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 62, 1, -1		-- 저온보관	(시작).
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 62, 2, -1		--			(즉시완료)
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 63, 1, -1		-- 정화시설	(시작).
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 63, 2, -1		--			(즉시완료)
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 64, 1, -1		-- 양동이	(시작).
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 64, 2, -1		--			(즉시완료)
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 65, 1, -1		-- 착유기	(시작).
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 65, 2, -1		--			(즉시완료)
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 66, 1, -1		-- 주입기	(시작).
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 66, 2, -1		--			(즉시완료)

-- 처음집테스트.
update dbo.tUserMaster set housestep = 0, housestate = 1, housetime = DATEADD(ss, 120, getdate()), gamecost = 1000, cashcost = 1000 where gameid = 'xxxx2'
exec spu_FacUpgrade 'xxxx2', '049000s1i0n7t8445289', 60, 2, -1		--			(즉시완료)
...
IF OBJECT_ID ( 'dbo.spu_FacUpgrade', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_FacUpgrade;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_FacUpgrade
	@gameid_								varchar(20),
...
Begin
	...
	select
		...
		@cashcost		= cashcost,			@gamecost		= gamecost,			@feedmax		= feedmax,
		@heartmax		= heartmax,			@fpointmax		= fpointmax,
	...
	if(@kind_ = @USERMASTER_UPGRADE_KIND_NEXT)
		begin
			...
			--select 'DEBUG > 단계세팅', @subcategory_ subcategory_, @nextstep nextstep, @itemcodesell itemcodesell, @cashcostsell cashcostsell, @gamecostsell gamecostsell, @upgradesecond upgradesecond
		...
		begin
			...
			from dbo.tItemInfo
			where subcategory = @subcategory_ and param1 = @nextstep
			--select 'DEBUG > 즉시완료', @subcategory_ subcategory_, @curstep curstep, @itemcodesell itemcodesell, @cashcostsell cashcostsell, @gamecostsell gamecostsell, @upgradesecond upgradesecond


			---------------------------------------------
			-- 5분에 2루비가격으로 결정.
			---------------------------------------------
			set @gaptime = dbo.fnu_GetDatePart( 's', getdate(), @curtime )
	...
	if(@gameid = '')
		...
		BEGIN
			select @gapsecond = dbo.fnu_GetDatePart('ss', @curtime, getdate())
	...
	--	> 업그레이드(next)에서 검사했으므로 여기는 논리적으로 안옴.
		...
		BEGIN
			...
			--select 'DEBUG ' + @comment
		END
	else if(@kind_ = @USERMASTER_UPGRADE_KIND_RIGHTNOW and @needcashcost > @cashcost)
		BEGIN
			set @nResult_ 	= @RESULT_ERROR_CASHCOST_LACK
				...
				begin
					...
					if(@curstate = @USERMASTER_UPGRADE_STATE_ING)
						begin
							...
							-- 구매기록마킹
							exec spu_UserItemBuyLogNew @gameid_, @itemcodesell, 0, @needcashcost, 0
