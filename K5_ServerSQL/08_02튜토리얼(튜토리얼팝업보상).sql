/*
delete from dbo.tTutoStep where gameid = 'xxxx3'
delete from dbo.tGiftList where gameid = 'xxxx3'
update dbo.tUserMaster set gamecost = 0, cashcost = 0, feed = 0, heart = 0, fpoint = 0 where gameid = 'xxxx3'
exec spu_TutoStep 'xxxx3', '049000s1i0n7t8445289', 5500, -1, -1	-- 튜토리얼 마킹
exec spu_TutoStep 'xxxx3', '049000s1i0n7t8445289', 5501, -1, -1	--
exec spu_TutoStep 'xxxx3', '049000s1i0n7t8445289', 5502, -1, -1	--
exec spu_TutoStep 'xxxx3', '049000s1i0n7t8445289', 5503, -1, -1	--
...
IF OBJECT_ID ( 'dbo.spu_TutoStep', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_TutoStep;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_TutoStep
	@gameid_								varchar(20),
...
Begin
	...
	if(@gameid = '')
		...
		END
	else if(@tutostep_ != -1 and exists(select top 1 * from dbo.tTutoStep where gameid = @gameid_ and itemcode = @tutostep_))
		BEGIN
			...
			if(@tutostep_ = -1)
				...
				begin
					if(exists(select top 1 * from dbo.tItemInfo where itemcode = @rewardvalue))
						begin
							exec spu_SubGiftSendNew @GIFTLIST_GIFT_KIND_GIFT, @rewardvalue, 0, 'SysTut', @gameid_, ''				-- 특정아이템 지급
						end
	...
	if(@nResult_ = @RESULT_SUCCESS and @brecord	= 1)
		BEGIN
			...
			if(@brewardwrite = @BOOL_TRUE)
				begin
					insert into dbo.tTutoStep(gameid,   itemcode,          ispass,                           gameyear,  gamemonth,  famelv)
