/*
update dbo.tUserMaster set fpoint = 1000, fmonth = 0 where gameid = 'xxxx2'
...
IF OBJECT_ID ( 'dbo.spu_CallFriendPoint', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_CallFriendPoint;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_CallFriendPoint
	@gameid_								varchar(20),
...
Begin
	...
	--	END
		...
		BEGIN
			...
			-- 사용로그 기록.
			exec spu_DayLogInfoStatic @market, 26, 1				-- 일 fpoint(무료)
		END
