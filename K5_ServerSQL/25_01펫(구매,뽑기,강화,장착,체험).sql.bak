...
/*
-- 펫 오늘만 이가격 추천구매.
--update dbo.tUserMaster set cashcost = 1000, gamecost = 10000, pettodayitemcode = -1 where gameid = 'xxxx3'
exec spu_ItemPet 'xxxx2', '049000s1i0n7t8445289', 1, 100000, -1	-- 펫구매(이미구매된것)
exec spu_ItemPet 'xxxx2', '049000s1i0n7t8445289', 1, 100001, -1

-- 펫 뽑기
exec spu_ItemPet 'xxxx2', '049000s1i0n7t8445289', 2,     -1, -1	-- 펫뽑기

-- 펫 업글
--update dbo.tUserMaster set gamecost = 10000 where gameid = 'xxxx3'
exec spu_ItemPet 'xxxx2', '049000s1i0n7t8445289', 3,      1, -1	--존재안함
exec spu_ItemPet 'xxxx2', '049000s1i0n7t8445289', 3,     40, -1

-- 펫 장착
--update dbo.tUserMaster set petcooltime = 5 where gameid = 'xxxx3'
exec spu_ItemPet 'xxxx2', '049000s1i0n7t8445289', 4,     40, -1
...
IF OBJECT_ID ( 'dbo.spu_ItemPet', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_ItemPet;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_ItemPet
...
Begin
	...
	if(@gameid = '')
		...
		BEGIN
			...
			if(@itemcode = -1)
				...
				BEGIN
					...
					--select 'DEBUG ' + @comment
				END
			else if(exists(select top 1 * from dbo.tUserItem where gameid = @gameid_ and itemcode = @itemcode and invenkind = @invenkind))
				BEGIN
					set @nResult_ = @RESULT_SUCCESS
					...
					-- 아이템 리스트인덱스
					select top 1 @listidxrtn = listidx from dbo.tUserItem where gameid = @gameid_ and itemcode = @itemcode and invenkind = @invenkind
				END
			else
				BEGIN
					set @nResult_ = @RESULT_SUCCESS
					...
					---------------------------------------------
					-- 펫구매 > 추가(초기업글)
					insert into dbo.tUserItem(gameid,      listidx,   itemcode, cnt,  invenkind,  petupgrade,      gethow)
					...
					-- 구매기록마킹
					exec spu_UserItemBuyLogNew @gameid_, @itemcode, 0, @pettodaycashcost, 0
		...
		BEGIN
			...
			-----------------------------------------------------
			-- 1차 랜덤잡기.
			select @petmax	= max(CAST(param10 as int)) from dbo.tItemInfo where subcategory = @ITEM_SUBCATEGORY_PET
			...
			if(@itemcode = -1)
				...
				BEGIN
					set @petupgrade		= -1
					select @petupgrade = petupgrade, @listidx = listidx from dbo.tUserItem
			...
			if(@petrollmode = @PET_ROLL_MODE_NON)
				...
				BEGIN
					...
					-- 구매기록마킹
					exec spu_UserItemBuyLogNew @gameid_, @itemcode, 0, @petrollcashcost, 0
				END
		END
	else if (@mode_ = @USERITEM_MODE_PET_UPGRADE)
		BEGIN
			set @listidx = @paramint_
			select @itemcode = itemcode, @petupgrade = petupgrade from dbo.tUserItem
		...
		BEGIN
			set @listidx = @paramint_
			select @itemcode = itemcode from dbo.tUserItem
			where gameid = @gameid_ and listidx = @listidx and invenkind = @invenkind
