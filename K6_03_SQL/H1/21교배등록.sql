...
exec spu_MPetRegister 'AAAA1', 1, '펫교배왕', -1		-- 존재하지않는 아이디
exec spu_MPetRegister 'DD78', 1, '펫교배왕', -1		-- 펫이 없음
exec spu_MPetRegister 'DD1', 1, '펫교배왕', -1		-- 등록/해제
exec spu_MPetRegister 'DD1', 0, '펫교배왕', -1		-- 등록/해제
*/
...
Begin
	...
	if isnull(@gameid, '') = ''
		...
		BEGIN
			...
			set @comment = 'ERROR 펫을 장착을 하지 않으셨습니다. 먼저 장착을 해주세요.'
		END
	else
		begin
			...
			if(@petregister_ = @MATE_PET_NOT)
				begin
					set @comment = '교배용 펫을 해제'
				end
			else
				begin
					set @comment = '교배용 펫을 등록' 
				end
