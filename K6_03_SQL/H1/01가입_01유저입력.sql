...
declare @ccode		int
declare @ukey		varchar(128)
declare @version	int
declare @phone		varchar(20)
declare @pushid		varchar(128)

-- 3-1. 커서선언
declare curSample Cursor for
select gameid, password, email, market, buytype, platform, ccode, ukey, version, phone, pushid 
from dbo.tSample 
where gameid not in (select gameid from dbo.tUserMaster where gameid in (select gameid from dbo.tSample))

-- 3-2. 커서오픈
open curSample

-- 3-3. 커서 사용
Fetch next from curSample into @gameid, @password, @email, @market, @buytype, @platform, @ccode, @ukey, @version, @phone, @pushid 
while @@Fetch_status = 0
	Begin	
		--select @gameid, @password, @email, @market, @buytype, @platform, @ccode, @ukey, @version, @phone, @pushid , -1
		exec spu_UserCreate @gameid, @password, @email, @market, @buytype, @platform, @ccode, @ukey, @version, @phone, @pushid , -1
		Fetch next from curSample into @gameid, @password, @email, @market, @buytype, @platform, @ccode, @ukey, @version, @phone, @pushid 
	end
