...
update dbo.tUserGameMTBaseball set buystate = -1 where gameid = 'xxxx2' and itemcode = 6930
exec spu_UserGameMTBaseball 'xxxx2', '049000s1i0n7t8445289',  1, 6901, -1		-- 구매
exec spu_UserGameMTBaseball 'xxxx2', '049000s1i0n7t8445289',  1, 6930, -1		-- 구매

update dbo.tUserGameMTBaseball set incomedate = getdate() - 0.1, buystate = 1 where gameid = 'xxxx2' and itemcode = 6900
exec spu_UserGameMTBaseball 'xxxx2', '049000s1i0n7t8445289',  2, 6900, -1		-- 수입

update dbo.tUserGameMTBaseball set incomedate = getdate() - 1 where gameid = 'xxxx2'
update dbo.tUserMaster set gamecost = 0 where gameid = 'xxxx2'
exec spu_UserGameMTBaseball 'xxxx2', '049000s1i0n7t8445289',  22, -1, -1		-- All수입

update dbo.tUserGameMTBaseball set incomedate = getdate() - 0.1, buystate = 1 where gameid = 'xxxx2' and itemcode = 6900
exec spu_UserGameMTBaseball 'xxxx2', '049000s1i0n7t8445289',  3, 6900, -1		-- 판매
*/
use GameMTBaseball
GO

IF OBJECT_ID ( 'dbo.spu_UserGameMTBaseball', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_UserGameMTBaseball;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_UserGameMTBaseball
...
Begin
	...
	if(@gameid = '')
		...
		BEGIN
			...
			if(@gamecost < @gamecostcur)
				...
				BEGIN
					...
					-- 목장 구매 상태로 변경
					---------------------------------------------
					--select 'DEBUG 전', @@ROWCOUNT, * from dbo.tUserGameMTBaseball where gameid = @gameid_ and itemcode = @itemcode_
					update dbo.tUserGameMTBaseball
						set
							...
							buycount	= buycount + 1
					where gameid = @gameid_ and itemcode = @itemcode_
					--select 'DEBUG 후', @@ROWCOUNT, * from dbo.tUserGameMTBaseball where gameid = @gameid_ and itemcode = @itemcode_
					...
					if(@rewarditemcode != -1)
						begin
							...
							exec spu_SubGiftSendNew @GIFTLIST_GIFT_KIND_GIFT,  @rewarditemcode, @rewardcnt, 'GameMTBaseballBuy', @gameid_, ''
						end
		...
		BEGIN
			...
			from
					(select itemcode, farmidx, incomedate, buycount from dbo.tUserGameMTBaseball where gameid = @gameid_ and buystate = @USERFARM_BUYSTATE_BUY) f
				JOIN
			...
			-- 3. 커서 사용
			Fetch next from curGameMTBaseballList into @farmidx, @incomedate, @buycount, @gamecostorg, @hourcoin, @maxcoin, @raiseyear, @raisepercent
	...
	if(@nResult_ = @RESULT_SUCCESS)
		begin
			...
			-- 농장리스트 전송
			--------------------------------------------------------------
			exec spu_UserGameMTBaseballListNew @gameid_, 1, @market, @version
