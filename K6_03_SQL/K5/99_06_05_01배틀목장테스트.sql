...
--update dbo.tUserGameMTBaseball set playcnt = 10 where gameid = (select top 1 gameid from dbo.tUserMaster where kakaouserid = @kakaouserid order by idx desc)
...
select 'DEBUG ', itemcode, playcnt from dbo.tUserGameMTBaseball where gameid = @gameid and buystate = 1 and playcnt > 0

-- 1. 정보 읽어서 선물하기.
declare curGameMTBaseballList Cursor for
select itemcode, playcnt from dbo.tUserGameMTBaseball
...
-- 3. 커서 사용
Fetch next from curGameMTBaseballList into @itemcode, @playcnt
