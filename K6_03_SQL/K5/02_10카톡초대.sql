/*
update dbo.tUserMaster set kakaomsginvitecnt = 9, kakaomsginvitetodaycnt = 0, kakaomsginvitetodaydate = kakaomsginvitetodaydate - 1 	where gameid = 'xxxx2'
update dbo.tUserMaster set kakaomsginvitetodaydate = kakaomsginvitetodaydate - 1 	where gameid = 'xxxx2'
update dbo.tKakaoInvite set senddate = senddate - 31 where gameid = 'xxxx2'
-- delete from dbo.tKakaoInvite where gameid = 'xxxx2'
-- delete from dbo.tGiftList where gameid = 'xxxx2'
exec spu_KakaoFriendInvite 'xxxx2',  '049000s1i0n7t8445289', 'kakaouseridxxxx', -1
...
IF OBJECT_ID ( 'dbo.spu_KakaoFriendInvite', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_KakaoFriendInvite;
GO

create procedure dbo.spu_KakaoFriendInvite
	@gameid_				varchar(20),
	@password_				varchar(20),
	@kakaouuid_				varchar(40),
	@nResult_				int				OUTPUT
...
Begin
	...
	--select 'DEBUG 1', @gameid_ gameid_, @password_ password_, @kakaouuid_ kakaouuid_
	...
	from dbo.tKakaoInvite
	where gameid = @gameid_ and receuuid = @kakaouuid_
	...
	if (@gameid = '')
		...
		begin
			...
			-- 일 카카오초대(O)
			-------------------------------------
			exec spu_DayLogInfoStatic @market, 15, 1


			------------------------------------------------
			-- 랭킹대전 정보수집.
			------------------------------------------------
			exec spu_subRankDaJun @gameid_, 0, 0, 0, 0, 10, 0, 0		-- 친구포인트

			--30일 이상이면 보낼수 있다.
			if(@sendedday = @SENDEDDAY_INIT)
				begin
					--select 'DEBUG 4-3 새로입력'
					insert into dbo.tKakaoInvite(gameid,   receuuid)
					values(                     @gameid_, @kakaouuid_)
				end
			else --if(@sendedday >= @SENDEDDAY_LIMIT)
				begin
			...
			if(@giftcode != @KAKAO_MSG_INVITE_STEP0_NON)
				begin
					...
					exec spu_SubGiftSendNew @GIFTLIST_GIFT_KIND_GIFT, @giftcode, 0, 'KakaoInv', @gameid_, ''				-- 특정아이템 지급

					select @subcategory = subcategory from dbo.tItemInfo where itemcode = @giftcode

					if(@subcategory = @ITEM_SUBCATEGORY_PET)
						begin
							Exec spu_SubGiftSendNew @GIFTLIST_GIFT_KIND_MESSAGE, -1, 0, 'KakaoInv', @gameid_, '펫은 우편함 수령후 [펫변경]에서 확인 합니다.'
						end
				end
			set @heart = @heart + 20
		end
