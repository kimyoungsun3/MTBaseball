...
while (@famelv < 80)
	begin
		select @famelv famelv,
			   dbo.fun_GetTreasurePrice( 100, @famelv) gamecost,-- 코인
			   dbo.fun_GetTreasurePrice( 101, @famelv) heart, 	-- 하트
			   dbo.fun_GetTreasurePrice( 102, @famelv) cash2, 	-- 캐쉬2
			   dbo.fun_GetTreasurePrice( 103, @famelv) cash4	-- 캐쉬4
		set @famelv = @famelv + 1
	end
...
IF OBJECT_ID ( N'dbo.fun_GetTreasurePrice', N'FN' ) IS NOT NULL
	DROP FUNCTION dbo.fun_GetTreasurePrice;
GO

CREATE FUNCTION dbo.fun_GetTreasurePrice(
	@mode_					int = 1,
	@famelv_				int = 1
)
	RETURNS int
...
BEGIN
	...
	if( @mode_ = @MODE_TREASURE_GRADE1_GAMECOST)
		begin
			--set @rtn = 200 + ( @famelv_ / 10) * ( @famelv_ / 10) * 100
			set @rtn = case
							when ( @famelv_ <=  9 ) then 400
							when ( @famelv_ <= 14 ) then 450
	...
	else if( @mode_ = @MODE_TREASURE_GRADE1_HEART)
		begin
			--set @rtn = 60 + ( @famelv_ / 10) * 10
			set @rtn = case
	...
	else if( @mode_ = @MODE_TREASURE_GRADE2_CASHCOST)
		begin
			set @rtn = 300
		end
	else if( @mode_ = @MODE_TREASURE_GRADE1_HEART)
