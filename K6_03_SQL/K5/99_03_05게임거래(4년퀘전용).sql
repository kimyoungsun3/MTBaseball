...
IF OBJECT_ID ( 'dbo.spu_GameTradeTest4', 'P' ) IS NOT NULL
    DROP PROCEDURE dbo.spu_GameTradeTest4;
GO

------------------------------------------------
--	1. 프로시져 생성
------------------------------------------------
create procedure dbo.spu_GameTradeTest4
	@gameid_								varchar(20),
...
as
	...
	declare @USER_LOG_MAX						int					set @USER_LOG_MAX 							= 12*5*1000 -- 12개월 * 5년.
		...
		BEGIN
			...
			if(@etestimate = 1 and @etgamemonth = @EPISODE_LOOP_MONTH - 1 and @gamemonth2 = @EPISODE_LOOP_MONTH)
				begin
					...
					else if(exists(select top 1 gameid from dbo.tUserGameMTBaseball where gameid = @gameid_ and itemcode = @etcheckvalue3 and buystate = @USERFARM_BUYSTATE_BUY))
						begin
			...
			-- if(연속거래성공)  거래 성공 누적
			------------------------------------------------
			if( @tradesuccess2 = @TRADEINFO_SUCCESS )
				begin
	...
	if(@nResult_ = @RESULT_SUCCESS)
		...
		BEGIN
			select @nResult_ rtn, @comment comment, @plusheartcow plusheartcow, @plusheartsheep plusheartsheep, @plusheartgoat plusheartgoat,
				   ...
				   @tradesuccesscnt tradesuccesscnt, @tradeclosedealer tradeclosedealer
