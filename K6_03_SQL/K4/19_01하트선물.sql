...
as
	...
	declare @state			int						set @state			= @USERFRIEND_STATE_NON

	-- 보낸이 하트
	declare @plusheart		int						set @plusheart		= 10
...
Begin
	...
	-- 3-3. 각 조건별 분기
	------------------------------------------------
	if (@mode_ not in (@USERFRIEND_MODE_HEARD, @USERFRIEND_MODE_RETURN_FRIEND))
		BEGIN
		...
		BEGIN
			...
			if(@gameid = '')
				...
				begin
					...
					--	유저정보.
					---------------------------------------------
					select 0 heartget2--, * from dbo.tFVUserMaster where gameid = @gameid_
