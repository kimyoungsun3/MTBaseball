...
update dbo.tFVUserMaster set salemoney = 12345678901232 where gameid = 'xxxx2'
...
Begin
	...
	if(@mode_ = 1)
		begin
			...
			-- 서버상태.
			----------------------------------------
			select top 1 @rtnflag = rtnflag from dbo.tFVSystemRouletteMan where roulmarket = (select market from dbo.tFVUserMaster where gameid = @gameid_) order by idx desc
			--select 'DEBUG', @rtnflag rtnflag
			...
			select isnull(bestani, 500) bestani, salemoney, m.gameid friendid, kakaotalkid, kakaouserid, kakaomsgblocked, kakaofriendkind, isnull(helpdate, getdate()), isnull(senddate, getdate()), isnull(rentdate, getdate()),
				...
				(select gameid, bestani, salemoney, kakaotalkid, kakaouserid, kakaomsgblocked, condate, logindate, rtndate from dbo.tFVUserMaster where gameid in (select friendid from @tTempFriendList) and kakaostatus = 1) as m
			JOIN
