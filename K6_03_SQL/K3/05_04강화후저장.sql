...
exec spu_FVTSUgrade 'xxxx2', '049000s1i0n7t8445289', 1, 80010, 0, 1,    0, 100, 'savedata1', 7776, -1			-- 일반강화(1)
exec spu_FVTSUgrade 'xxxx2', '049000s1i0n7t8445289', 2, 80010, 0, 1, 1000,   0, 'savedata2', 7777, -1			-- 결정강화(2)
			...
			if(@mode_ = @MODE_TSUPGRADE_NORMAL)
				...
				begin
					...
					if(@ownercashcost2 > @ownercashcost)
						begin
							set @strange = 1
							set @comment2 = '강화이상 보유(' + ltrim(rtrim(@ownercashcost)) + ') < 현재비용(' + ltrim(rtrim(@ownercashcost2)) + ')'
							--select 'DEBUG **** 이상기록', @comment2 comment2
							exec spu_FVSubUnusualRecord2  @gameid_, @comment2
						end
				end
