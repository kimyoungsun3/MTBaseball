/*
update dbo.tFVUserMaster set bestdealer = 12345678901230 where gameid = 'xxxx'
update dbo.tFVUserMaster set bestdealer = 12345678901232 where gameid = 'xxxx2'
update dbo.tFVUserMaster set bestdealer = 12345678901233 where gameid = 'xxxx3'

exec spu_FVsubFriendRank 'xxxx2', 1		-- 랭킹
exec spu_FVsubFriendRank 'xxxx2', 0		-- Empty
...
Begin
	...
	if(@mode_ = 1)
		begin
			...
			select isnull(bestani, 10001) bestani, bestdealer, m.gameid friendid, kakaotalkid, kakaouserid, kakaomsgblocked, kakaofriendkind, isnull(helpdate, getdate()), isnull(senddate, getdate()), isnull(rentdate, getdate()),
				case
				...
				(select gameid, bestani, bestdealer, kakaotalkid, kakaouserid, kakaomsgblocked, condate, rtndate from dbo.tFVUserMaster where gameid in (select friendid from @tTempFriendList) and kakaostatus = 1) as m
			JOIN
			...
			-- 친구 리스트를 갱신
			select rank() over(order by bestdealer desc) as rank, * from @tTempTableList order by bestdealer desc
