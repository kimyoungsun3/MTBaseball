...
create table dbo.tFVUserMaster(
	...
	savebktime	datetime				default(getdate() - 1),
	...
	researchpoint int					default(0),				-- 누적연구포인트.
	...
	-- 랭킹정보.
	bestani			int					default(10001),
	bestdealer		int					default(0),				-- 최고상인거래.
	newstart		int					default(0),				-- 새로하기 카운터.
	...
	rkbestdealerbk	bigint				default(0),				-- 백업
	...
	bestdealerbkup	bigint				default(0),				-- 누적판매금액백엄(스케쥴에 의해서백업).
	lmbestdealer	bigint				default(0),				-- 지난 내점수.
	...
	roulfreetimetotal	int				default(0),				-- 무료 시간 뽑기 (12, 18, 21).
	roulfreetimedate	varchar(8)		default('20100101'),
	roulfreetimehour	int				default(0),
	roulfreehearttotal	int				default(0),				-- 무료 하트 뽑기
	roulfreeheartdate	varchar(8)		default('20100101'),
	roulfreeheartcnt	int				default(0),
	roulcashcosttotal	int				default(0),				-- 캐쉬뽑기
	roulcashcostgauage	int				default(0),				--          (이벤트 게이지).
	roulcashcostfree	int				default(0),				--          (이벤트 표시).
	roulcashcostfreetotal		int		default(0),				--          (이벤트 수량).
	roulcashcost2total			int		default(0),				-- 캐쉬뽑기2
	roulcashcost2gauage			int		default(0),				-- 			(이벤트 게이지).
	roulcashcost2free			int		default(0),				--          (이벤트 표시).
	roulcashcost2freetotal		int		default(0),				--          (이벤트 수량).
	roulfreetickettotal			int		default(0),				-- 무료티켓
	roulcashcosttickettotal		int		default(0),				-- 유료티켓
	roulcashcost2tickettotal	int		default(0),				-- 유료티켓2
	...
	wheeldayfree	int					default(1),				-- 1일 1회 무료 돌리기.
	...
	-- 캐쉬구매후 30일간 지급.
	cashdatestart	smalldatetime		default('20000101'),
	cashdateend		smalldatetime		default('20000101'),
	cashdatecur		smalldatetime		default('20000101'),
...
IF EXISTS (SELECT name FROM sys.indexes WHERE name = N'idx_tFVUserMaster_bestdealer')
   DROP INDEX tFVUserMaster.idx_tFVUserMaster_bestdealer
GO
CREATE INDEX idx_tFVUserMaster_bestdealer ON tFVUserMaster (bestdealer)
GO
...
-- insert into dbo.tFVUserMaster(gameid, phone, market, version) values('xxxx2', '01022223333', 5, 101)
-- select * from dbo.tFVUserMaster where gameid = 'xxxx2'
-- update dbo.tFVUserMaster set market = 5, version = 101 where gameid = 'xxxx2'
...
create table dbo.tFVDayLogInfoStatic(
	...
	gamerewardcnt 	int				default(0),					-- 게임 무료충전카운터.(배너형태)
...
create table dbo.tFVEventSub(
	...
	eventsender		varchar(60)			default('일일보상'),
...
create table dbo.tFVRouletteLogTotalMaster(
	...
	roulfreetimetotal			int			default(0),			-- 보물뽑기.
	roulfreehearttotal			int			default(0),
	roulcashcosttotal			int			default(0),
	roulcashcost2total			int			default(0),
