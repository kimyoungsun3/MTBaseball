...
<table>
	...
	<tr>
		<td align="center">
			<form name="GIFTFORM" method="post" action="roulettelogtotal_list.jsp" onsubmit="return f_Submit(this);">
				...
				<table border=1>
					...
					<%while(result.next()){%>
						<tr>
							...
							<td><a href=roulettelogtotal_list.jsp?dateid=<%=result.getString("dateid")%>><%=result.getString("dateid")%></a></td>
							<td><%=result.getString("goldballkind")%></td>
							<td><%=result.getString("cashcost")%></td>
							<td><%=result.getString("gamecost")%></td>
							<%if(dateid.equals("")){%>
								<td><%=result.getString("itemcodecnt")%></td>
							<%}else{%>
