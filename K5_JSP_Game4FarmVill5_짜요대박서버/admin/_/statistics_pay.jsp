...
<table>
	...
	<tr>
		<td align="center">
			<form name="GIFTFORM" method="post" action="statistics_pay.jsp" onsubmit="return f_Submit(this);">
			<div  style="border:1px solid #D7D6D6;background:#FCFCFC;padding:36px 0;">
				<table>
					<tr>
						<td colspan=4> 
						유저 유료가입자 체킹(와일드검색은 100건까지)
						</td>
					</tr>
					<tr>
						<td> 폰번호[<%=phone%>]</td>
				...
				<table border=1>
					<%
					//2. 데이타 조작
					//exec spu_FarmD 21, -1, -1, 60, -1, -1, -1, -1, -1, -1, '', '', '', '', ''			-- 유료가입자 유니크 체크
					//exec spu_FarmD 21, -1, -1, 60, -1, -1, -1, -1, -1, -1, '', '01022223333', '', '', ''	
					...
					<%while(result.next()){%>
						<tr>
							...
							<td><a href=statistics_pay.jsp?phone=<%=result.getString("phone")%>><%=result.getString("phone")%></a></td>
							<td><%=getTel(result.getInt("market"))%></td>
							<td><%=result.getString("writedate")%></td>
							<td>
								<%if(bAdmin && !bExpire){%>
									<a href=userinfodel_ok.jsp?pagebranch=statistics_pay&kind=61&idx=<%=result.getString("idx")%>>개발삭제</a>
								<%}%>
							</td>
						</tr>
					<%}%>
