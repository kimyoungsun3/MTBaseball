	...
	<tr>
		<td align="center">
			...
			<div  style="border:1px solid #D7D6D6;background:#FCFCFC;padding:36px 0;">
				<table>
					<tr>
						<td colspan=4> 일별 캐쉬 구매(일자별 구매내역별 정렬)</td>
					</tr>
					<tr>
						<td> 구매날자[<%=dateid2%>]</td>
						<td> 검색</td>
				...
				<table border=1>
					<%
					//2. 데이타 조작
					//exec spu_FarmD 21, -1, -1, 0, -1, -1, -1, -1, -1, -1, '', '', '', '', ''		--캐쉬토탈(0)
					//exec spu_FarmD 21, -1, -1, 0, -1, -1, -1, -1, -1, -1, '', '20120819', '', '', ''
					query.append("{ call dbo.spu_FarmD (?, ?, ?, ?, ?,   ?, ?, ?, ?, ?,   ?, ?, ?, ?, ?)} ");
					cstmt = conn.prepareCall(query.toString());
					cstmt.setInt(idxColumn++, 21);
					cstmt.setInt(idxColumn++, -1);
					cstmt.setInt(idxColumn++, -1);
					cstmt.setInt(idxColumn++, 0);
					cstmt.setInt(idxColumn++, -1);
					cstmt.setInt(idxColumn++, -1);
					cstmt.setInt(idxColumn++, -1);
					cstmt.setInt(idxColumn++, -1);
					...
					while(result.next()){%>
						<%
							...
							if(sumDate != result.getInt("dateid")){%>
								<tr><td colspan=7 align=right><%=loopSum%></td></tr>
								<%
								loopSum = 0;
								loopPage = 1;
								sumDate = result.getInt("dateid");
