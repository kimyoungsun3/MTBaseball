				...
				<table>
					...
					<tr>
						<td colspan=2>
							...
							<a href=sysinquire_list.jsp><img src=images/refresh2.png alt="화면갱신"></a></td>
					</tr>
					<tr>
						<td>
							<input name="gameid" type="text" value="<%=gameid%>" size="60" maxlength="256" tabindex="1" style="border:1px solid #EBEBEB;background:#FFFFFF;">
					...
					<%while(result.next()){%>
						<tr <%=getInQuireStateColor(result.getInt("state"))%>>
							...
							<td>
								...
								<a href=userroullog_list.jsp?gameid=<%=result.getString("gameid")%> target=_blank>보물뽑기</a><br>
								<a href=cashbuy_list.jsp?gameid=<%=result.getString("gameid")%> target=_blank>캐쉬검사</a><br>
								<a href=cash_form.jsp?gameid=<%=result.getString("gameid")%> target=_blank>캐쉬강제입력(캐쉬선물로발송됨)</a><br>
								<a href=wheel_list.jsp?gameid=<%=result.getString("gameid")%> target=_blank>회전판</a><br>
								<a href=unusual_list.jsp?gameid=<%=result.getString("gameid")%> target=_blank>비정상검색(일반)</a><br>
								<a href=unusual_list2.jsp?gameid=<%=result.getString("gameid")%> target=_blank>비정상검색(로그만)</a><br>
							</td>
							<%
