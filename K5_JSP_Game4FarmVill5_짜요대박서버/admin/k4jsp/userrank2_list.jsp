		...
		<td align="center">
			...
			<div  style="border:1px solid #D7D6D6;background:#FCFCFC;padding:36px 0;">
				<table>
					<tr>
						<td colspan=4>
							1. 환생 자체를 랭킹이 기록됩니다.(월, 목, 토)<br>
							100위 랭킹 <a href=userrank2_list.jsp><img src=images/refresh2.png alt="화면갱신"></a>
						</td>
					</tr>
				</table>
				<table border=1>
					<%
					//2. 데이타 조작
					//exec spu_FVFarmD 19, 100, -1, -1, -1, -1, -1, -1, -1, -1, '', '', '', '', '', '', '', '', '', ''						-- 상위랭킹 100위
					query.append("{ call dbo.spu_FVFarmD (?, ?, ?, ?, ?,   ?, ?, ?, ?, ?,   ?, ?, ?, ?, ?,   ?, ?, ?, ?, ?)} ");
					cstmt = conn.prepareCall(query.toString());
					cstmt.setInt(idxColumn++, 19);
					cstmt.setInt(idxColumn++, 100);
					...
					<%while(result.next()){%>
						<tr>
							...
							<td><%=displayMoney(result.getString("rebirthpts"))%></td>
							<td><%=displayMoney(result.getString("rebirthptscnt"))%></td>
