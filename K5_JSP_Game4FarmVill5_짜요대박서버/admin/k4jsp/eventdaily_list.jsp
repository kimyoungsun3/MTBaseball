...
<%
	...
	try{
		//2. 데이타 조작
		//exec spu_FVFarmD 30, 50, -1, -1, -1, -1, -1, -1, -1, -1, '', '', '', '', '', '', '', '', '', ''		-- 시간제 이벤트 정보.
		...
		if(cstmt.getMoreResults()){
			...
			while(result.next()){
				itemdesc[k] = result.getString("itemname") + "x"
			...
			<div  style="border:1px solid #D7D6D6;background:#FCFCFC;padding:36px 0;">
				<table>
					...
					<tr>
						<td>선물자</td>
						<td>
							<input name="ps3" type="text" value="sangsang" size="20" maxlength="20" tabindex="1" style="border:1px solid #EBEBEB;background:#FFFFFF;">
						</td>
					</tr>
					<tr>
						<td>날짜</td>
				...
				<table border=1>
					...
					<%if(cstmt.getMoreResults()){
						...
						while(result.next()){
							%>
						<tr <%=getCheckValueOri(result.getInt("eventstatedaily"), 0, "bgcolor=#aaaaaa", "")%> <%=getCheckValueOri(result.getInt("eventidx"), eventidx, "bgcolor=#ffe020", "")%> >
								<form name="GIFTFORM" method="post" action="usersetting_ok.jsp" onsubmit="return f_Submit(this);">
								<input type="hidden" name="branch" value="eventdaily_list">
								...
								<td>
									<select name="p6" >
										<option value="-1" <%=getSelect(result.getInt("eventstatedaily"), -1)%>>비활성(0)</option>
										<option value="1" <%=getSelect(result.getInt("eventstatedaily"), 1)%>>  활성(1)</option>
									</select>
								</td>
								<td>
									<input name="p5" type="text" value="<%=result.getInt("eventitemcode")%>" size="5" maxlength="5" tabindex="1" style="border:1px solid #EBEBEB;background:#FFFFFF;"><br>
									<%=result.getString("itemname")%>
									<input name="p7" type="text" value="<%=result.getInt("eventcnt")%>" size="3" maxlength="5" tabindex="1" style="border:1px solid #EBEBEB;background:#FFFFFF;">개
								</td>
								<td>
