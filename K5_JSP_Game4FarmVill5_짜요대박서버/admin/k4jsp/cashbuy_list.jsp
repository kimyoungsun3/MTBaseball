		...
		<td align="center">
			<div  style="border:1px solid #D7D6D6;background:#FCFCFC;padding:36px 0;">
				<table>
					<tr>
						<td colspan=3>
							수정(캐쉬)구매 내역(유저기준 검색) > 검색을 하면 전체 내역이 나옵니다.<br>
				...
				<table border=1>
					<%
					//2. 데이타 조작
					//exec spu_FVFarmD 21, 11,-1, -1, -1, -1, -1, -1, -1, -1, '', '', '', '', '', '', '', '', '', ''						-- 수정(캐쉬)판매로그
					//exec spu_FVFarmD 21, 11, 1, -1, -1, -1, -1, -1, -1, -1, '', '', '', '', '', '', '', '', '', ''						--
					//exec spu_FVFarmD 21, 11,10, -1, -1, -1, -1, -1, -1, -1, '', '', '', '', '', '', '', '', '', ''						--
					//exec spu_FVFarmD 21, 11,10, -1, -1, -1, -1, -1, -1, -1, 'xxxx', '', '', '', '', '', '', '', '', ''					--
					...
					<%while(result.next()){%>
						<tr>
							...
							<td>
								<%=getTel(result.getInt("market"))%>(<%=result.getString("ikind")%>)
								<%=result.getString("concode")%>
							</td>
							<td>
								<a href=cashbuy_list.jsp?gameid=<%=result.getString("gameid")%>><%=result.getString("gameid")%></a>
								(<%=result.getString("kakaouserid")%>)
							</td>
							<td><a href=userinfo_list.jsp?gameid=<%=getGameid(result.getString("giftid"))%> target=_blank><%=getGameid(result.getString("giftid"))%></a></td>
							<td><a href=cashbuy_list.jsp?acode=<%=result.getString("acode")%>><%=getShotMsg(result.getString("acode"), 60)%></td>
							...
							<td>
								<a href=cash_modify.jsp?p1=17&p2=3&p3=<%=result.getString("idx")%>&p4=<%=result.getString("cash")%>&ps1=<%=result.getString("gameid")%>&ps2=<%=adminid%>&gameid=<%=result.getString("gameid")%>&branch=cashbuy_list&title=cashmodify&ps4=<%=result.getString("acode")%> target=_blank>
									영수증수정
								</a>
								&nbsp;
					...
					<%}%>
					<tr>
						<td colspan=15> 총캐쉬:<%=String.format("%,d", cashtotal)%></td>
					</tr>
