...
<table>
	...
	<tr>
		<td align="center">
			<form name="GIFTFORM" method="post" action="tsupgrade_list.jsp" onsubmit="return f_Submit(this);">
			<div  style="border:1px solid #D7D6D6;background:#FCFCFC;padding:36px 0;">
				<table>
					<tr>
						<td colspan=3>
							강화정보 <a href=tsupgrade_list.jsp><img src=images/refresh2.png alt="화면갱신"></a><br>
						</td>
					</tr>
					<tr>
						<td>검색</td>
				...
				<table border=1>
					...
					<%while(result.next()){%>
						<tr>
							...
							<td><a href=tsupgrade_list.jsp?gameid=<%=result.getString("gameid")%>><%=result.getString("gameid")%></a></td>
							...
							<td>
								<a href=usersetting_ok.jsp?p1=19&p2=1031&p3=<%=result.getString("idx")%>&ps1=<%=result.getString("gameid")%>&branch=tsupgrade_list&gameid=<%=result.getString("gameid")%>><%=getTSStrange(result.getInt("strange"))%></a>
							</td>
							...
							<td>
								...
								<a href=usersetting_ok.jsp?p1=19&p2=1011&p3=<%=result.getString("idx")%>&ps1=<%=result.getString("gameid")%>&ps2=<%=adminid%>&branch=tsupgrade_list&gameid=<%=result.getString("gameid")%>>
									삭제
								</a>
								<a href=usersetting_ok.jsp?p1=19&p2=1012&p3=<%=result.getString("idx")%>&ps1=<%=result.getString("gameid")%>&ps2=<%=adminid%>&branch=tsupgrade_list&gameid=<%=result.getString("gameid")%>>
									일괄삭제
								</a>
							</td>
						</tr>
					...
					<tr>
						<td colspan=13 align=center>
								<a href=tsupgrade_list.jsp?idxPage=<%=(idxPage - 1 < 1)?1:(idxPage-1)%>><<</a>
								<%=idxPage%> / <%=maxPage%>
								<a href=tsupgrade_list.jsp?idxPage=<%=(idxPage + 1 > maxPage)?maxPage:(idxPage + 1)%>>>></a>
						</td>
					</tr>
				</table>
			</div>
