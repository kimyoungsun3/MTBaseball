			...
			<div  style="border:1px solid #D7D6D6;background:#FCFCFC;padding:36px 0;">
				<table>
					<tr>
						<td colspan=4>
							세부 내역아이템구매날자[<%=dateid%>]
							<a href=usertreasurelogtotal_mas.jsp>(돌아가기)</a>
							<a href=usertreasurelogtotal_sub.jsp?dateid=<%=dateid%>&view=1>(이미지보기)</a>
							<a href=usertreasurelogtotal_sub.jsp?dateid=<%=dateid%>&view=0>(노이미지)</a>
						</td>
					</tr>
				</table>
			</div>
				...
				<table border=1>
					<%
					//2. 데이타 조작
					//exec spu_GameMTBaseballD 19, 432,-1, -1, -1, -1, -1, -1, -1, -1, '', '20131111', '', '', '', '', '', '', '', ''				-- 유저 뽑기월서브 통계
					query.append("{ call dbo.spu_GameMTBaseballD (?, ?, ?, ?, ?,   ?, ?, ?, ?, ?,   ?, ?, ?, ?, ?,   ?, ?, ?, ?, ?)} ");
					cstmt = conn.prepareCall(query.toString());
					cstmt.setInt(idxColumn++, 19);
					cstmt.setInt(idxColumn++, 432);
